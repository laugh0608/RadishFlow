...
- 在这层 bridge 之上，当前又继续补出 `UnitOperationHostActionExecutionDispatcher`：宿主现在可以直接提交 `UnitOperationHostActionExecutionRequest`，由库内统一决定是返回 `LifecycleOperationRequired / Unsupported`，还是把 parameter/port action 落到 `UnitOperationHostObjectMutationDispatcher.DispatchBatch(...)` 并回传 ordered mutation outcomes、applied mutation count 与 invalidation 摘要；这样“读 action plan -> 翻译 -> 执行”这条最小宿主驱动链已经在库内闭环
...
- 当前“宿主如何驱动 PMC”这条最小 orchestration helper 仍故意留在 `RadishFlow.CapeOpen.SmokeTests`：`UnitOperationSmokeHostDriver` 现在已经开始通过 `UnitOperationHostActionExecutionDispatcher` 应用 parameter/port 类阻塞配置动作，并继续用直接参数/端口改写覆盖非法状态和 stale 状态边界；是否把 driver 上移到库内仍等 smoke 侧调用顺序、最小必需输入和失败分类继续稳定后再决定，`UnitOp.Mvp` 本身当前仍只承诺 PMC 对象面、action execution helper 与结果读取/展示 helper，不提前承诺更高层宿主驱动 convenience API
