...
- `UnitOperationHostActionExecutionOrchestrator`、`UnitOperationHostActionExecutionOrchestrationResult` 与 `UnitOperationHostActionExecutionFollowUp`，用于把 request planning、action execution、刷新后的 configuration/action plan/session 读取，以及“宿主下一步该做什么”继续收口成一条窄边界 orchestration helper：宿主可一次得到 planned action count、ready request count、missing inputs、mutation invalidation 摘要、执行后的最新 host views，以及 `LifecycleOperation / ProvideInputs / Validate / Calculate / Terminated` 五类 follow-up，但该 helper 仍不负责 `Initialize / Validate / Calculate / Terminate`
...
- 在这层 bridge 之上，当前又继续补出 `UnitOperationHostActionExecutionRequestPlanner`、`UnitOperationHostActionExecutionDispatcher` 与 `UnitOperationHostActionExecutionOrchestrator`：planner 只把 action plan 和宿主显式提供的输入规划成 executable request plan，并清晰暴露缺失输入、lifecycle-only 与 unsupported action；dispatcher 再消费 ready requests，统一决定是返回 `LifecycleOperationRequired / Unsupported`，还是把 parameter/port action 落到 `UnitOperationHostObjectMutationDispatcher.DispatchBatch(...)` 并回传 ordered mutation outcomes、applied mutation count 与 invalidation 摘要；orchestrator 则在执行后继续刷新 configuration/action plan/session 三块正式 host view。这样“读 action plan -> 准备宿主输入 -> 规划 execution request -> 执行 -> 刷新宿主视图”这条最小宿主配置动作链已在库内收口，但仍不把 smoke 专用默认输入或完整 driver 生命周期上移成正式 API
