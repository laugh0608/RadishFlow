...
- `ConfigureNativeLibraryDirectory(...)`、`LastCalculationResult`、`LastCalculationFailure`、`GetCalculationReport()`、`GetCalculationReportState()`、`GetCalculationReportHeadline()`、`GetCalculationReportDetailValue(string)`、`GetCalculationReportLineCount()`、`GetCalculationReportLine(int)`、`GetCalculationReportLines()` 与 `GetCalculationReportText()`
...
- 在该元数据面之上，当前又补出 `GetCalculationReportDetailValue(string)` 这一条最小 detail 键值读取入口，让宿主可直接读取稳定的单值 detail key，例如 `status`、`highestSeverity`、`diagnosticCount`、`error`、`operation`、`requestedOperation`、`nativeStatus` 与 `diagnosticCode`
