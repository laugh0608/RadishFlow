...
- 在这层 bridge 之上，当前又继续补出 `UnitOperationHostActionExecutionRequestPlanner` 与 `UnitOperationHostActionExecutionDispatcher`：前者只把 action plan 和宿主显式提供的输入规划成 executable request plan，并清晰暴露缺失输入、lifecycle-only 与 unsupported action；后者再消费 ready requests，统一决定是返回 `LifecycleOperationRequired / Unsupported`，还是把 parameter/port action 落到 `UnitOperationHostObjectMutationDispatcher.DispatchBatch(...)` 并回传 ordered mutation outcomes、applied mutation count 与 invalidation 摘要。这样“读 action plan -> 准备宿主输入 -> 规划 execution request -> 执行”这条最小宿主配置动作链已在库内收口，但仍不把 smoke 专用默认输入或完整 driver 生命周期上移成正式 API
