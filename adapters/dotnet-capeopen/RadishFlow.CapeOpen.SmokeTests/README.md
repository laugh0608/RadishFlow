...
- `UnitOp.Mvp` 模式可校验最小成功结果契约 `status / summary / diagnostics`、失败路径的最小诊断摘要契约、统一只读查询面 `GetCalculationReport()`，以及建立在其上的最小标量/文本消费面 `GetCalculationReportState()` / `GetCalculationReportHeadline()` / `GetCalculationReportLineCount()` / `GetCalculationReportLine(int)` / `GetCalculationReportLines()` / `GetCalculationReportText()`
