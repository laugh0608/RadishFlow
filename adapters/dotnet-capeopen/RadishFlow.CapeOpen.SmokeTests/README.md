...
- `UnitOp.Mvp` 模式可校验最小成功结果契约 `status / summary / diagnostics`、失败路径的最小诊断摘要契约、统一只读查询面 `GetCalculationReport()`，以及建立在 report DTO 之上的最小文本导出面 `GetCalculationReportLines()` / `GetCalculationReportText()`
