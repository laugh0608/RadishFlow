...
- 求解输出中的关键流股状态断言
- 非 flash 中间流股进入后续 flash inlet 时的 `phase_region` / `bubble_dew_window` near-boundary `±ΔP / ±ΔT` 稳定性回归，当前同时覆盖 `binary-hydrocarbon-lite-v1` three-composition two-phase 与 synthetic `liquid-only / vapor-only` 单相样例
