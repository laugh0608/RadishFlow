...
#[test]
fn studio_solver_bridge_preserves_synthetic_single_phase_pressure_near_boundary_windows_across_heater_flash_inlet_end_to_end()
 {
    assert_near_boundary_cases_across_chain(
        synthetic_single_phase_near_boundary_stream_window_cases()
            .into_iter()
            .filter(|case| case.kind == NearBoundaryCaseKind::Pressure)
            .collect(),
        "snapshot-studio-synthetic-heater-pressure",
        "heater-1",
        &["stream-feed"],
        "stream-heated",
        |index, case| {
            app_state_for_synthetic_heater_boundary_case(
                &format!("doc-studio-synthetic-heater-pressure-{index}"),
                &case.label,
                720 + index as u64,
                case,
            )
...
#[test]
fn studio_solver_bridge_preserves_synthetic_single_phase_temperature_near_boundary_windows_across_heater_flash_inlet_end_to_end()
 {
    assert_near_boundary_cases_across_chain(
        synthetic_single_phase_near_boundary_stream_window_cases()
            .into_iter()
            .filter(|case| case.kind == NearBoundaryCaseKind::Temperature)
            .collect(),
        "snapshot-studio-synthetic-heater-temperature",
        "heater-1",
        &["stream-feed"],
        "stream-heated",
        |index, case| {
            app_state_for_synthetic_heater_boundary_case(
                &format!("doc-studio-synthetic-heater-temperature-{index}"),
                &case.label,
                760 + index as u64,
                case,
            )
...
fn studio_solver_bridge_preserves_synthetic_single_phase_pressure_near_boundary_windows_across_cooler_flash_inlet_end_to_end()
 {
    assert_near_boundary_cases_across_chain(
        synthetic_single_phase_near_boundary_stream_window_cases()
            ...
            .filter(|case| case.kind == NearBoundaryCaseKind::Pressure)
            .collect(),
        "snapshot-studio-synthetic-cooler-pressure",
        "cooler-1",
        &["stream-feed"],
        "stream-cooled",
        |index, case| {
            app_state_for_synthetic_cooler_boundary_case(
                &format!("doc-studio-synthetic-cooler-pressure-{index}"),
                &case.label,
                800 + index as u64,
                case,
            )
...
fn studio_solver_bridge_preserves_synthetic_single_phase_temperature_near_boundary_windows_across_cooler_flash_inlet_end_to_end()
 {
    assert_near_boundary_cases_across_chain(
        synthetic_single_phase_near_boundary_stream_window_cases()
            ...
            .filter(|case| case.kind == NearBoundaryCaseKind::Temperature)
            .collect(),
        "snapshot-studio-synthetic-cooler-temperature",
        "cooler-1",
        &["stream-feed"],
        "stream-cooled",
        |index, case| {
            app_state_for_synthetic_cooler_boundary_case(
                &format!("doc-studio-synthetic-cooler-temperature-{index}"),
                &case.label,
                840 + index as u64,
                case,
            )
...
fn studio_solver_bridge_preserves_synthetic_single_phase_pressure_near_boundary_windows_across_valve_flash_inlet_end_to_end()
 {
    assert_near_boundary_cases_across_chain(
        synthetic_single_phase_near_boundary_stream_window_cases()
            ...
            .filter(|case| case.kind == NearBoundaryCaseKind::Pressure)
            .collect(),
        "snapshot-studio-synthetic-valve-pressure",
        "valve-1",
        &["stream-feed"],
        "stream-throttled",
        |index, case| {
            app_state_for_synthetic_valve_boundary_case(
                &format!("doc-studio-synthetic-valve-pressure-{index}"),
                &case.label,
                880 + index as u64,
                case,
            )
...
fn studio_solver_bridge_preserves_synthetic_single_phase_temperature_near_boundary_windows_across_valve_flash_inlet_end_to_end()
 {
    assert_near_boundary_cases_across_chain(
        synthetic_single_phase_near_boundary_stream_window_cases()
            ...
            .filter(|case| case.kind == NearBoundaryCaseKind::Temperature)
            .collect(),
        "snapshot-studio-synthetic-valve-temperature",
        "valve-1",
        &["stream-feed"],
        "stream-throttled",
        |index, case| {
            app_state_for_synthetic_valve_boundary_case(
                &format!("doc-studio-synthetic-valve-temperature-{index}"),
                &case.label,
                920 + index as u64,
                case,
            )
