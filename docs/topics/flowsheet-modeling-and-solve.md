...
| Heater / Cooler 温压调节单元 | Active | `unitops/heater-cooler.md` |
...
2026-09-16 确认将从工程目标反推操作条件纳入稳态工程能力。此处的 Design Specification 是求解任务，区别于 [规格完整性检查](modeling-assistance-and-specifications.md)：检查判断条件是否足够 / 冲突，求解实际调整变量以达到目标。

任务由目标量及目标值、可调整输入、上下限、所选工况、固定条件、方法和容差组成。示例包括给定出口温度求公用工程流量、给定分离指标调整操作条件、在设备约束内研究处理能力。例子需对应真实模型；当前目标 T/P Heater 或演示 Flash 不代表这些反算已经可用。模型中必须释放相应规格，不能一面固定输入一面声明由求解器调整。

- 只允许调整明确可写且被选入任务的工程输入；目标输出仍只读，不能直接把结果改成目标值。
- 先验证目标与可调变量的依赖、单位 / 基准、规格独立性和上下限，再在冻结工况 / 输入副本上求解。试算不逐次写入活动文档或 Undo 历史。
- 首版可选一个可调变量与一个目标残差，使用适合模型的有界求根路径；存在回路时还需保证内层求解状态正确。数值方法随案例确认，不提前引入完整优化器。
- 返回实际目标残差、约束余量、迭代 / 内层失败信息和解的适用工况。用户显式采纳后经正式事务写入；源输入已变化时拒绝过期采纳。
- 一个可行解不等于最优或唯一解。多根须说明所采用的区间 / 初值或选择依据；在限定搜索范围未找到解、模型超域、数值失败、已证实规格冲突分别返回，不能将求解失败冒充无可行解证明。
- 最大处理量或最经济方案通常需要约束优化；不能凭一次目标求根或一次扫描直接宣称全局最大 / 最优。

| 切片 | 范围 | 退出标准 |
| --- | --- | --- |
| DS0 | 选定首个真实目标 / 可调变量、模型和独立基准 | 规格可释放、单位与边界清楚；与直接计算和检查任务区分 |
| DS1 | 有界单目标反算、诊断、候选采纳 | 已知解和边界工况一致；不可达 / 内层失败 / 多根选择可解释；试算不污染文档 |
| DS2 | 随设备和模型扩展多变量或约束优化 | 明确可行 / 局部最优 / 搜索失败语义；有独立基准及必要的敏感性检验 |

DS0 / DS1 在 P3 随模型能力进入，不等待完整 EO 或通用优化。验证覆盖已知反算、单位等价、边界 / 超域、无依赖目标、相关规格、内层失败、多根、源修订变化、采纳 / 撤销与保存重开。工况和来源复用 [工程基础](engineering-basis-and-cases.md)，结果诊断和输出复用结果专题；不在界面新增私有求解器。
...
- 实际建模修改必须进入 `DocumentCommand` / command history；GUI 与统一变量写入共享输入事务，相同有效输入不新增历史或修订。
- 应用内创建 / 连接 / 运行复用既有路径；连接执行重建本地候选并校验端点，不把候选生成当作完整拓扑验证。调用前置条件见 [B3-3](studio-main-workflow.md#b3-3受控创建连接与运行调用)；CLI 当前仅接入已有项目参数写入与运行，尚不接受创建 / 连接请求。
...
2026-09-14 已实现并通过 macOS 实窗验收。上节 2026-09-13 的复核表保留当时事实，其中平台标签与 macOS 标准重做缺口由本节覆盖；Windows / Linux 原生窗口仍待验证。

[快捷键模型](../../apps/radishflow-studio/src/studio_gui_command_registry/shortcuts.rs) 显式区分 `Primary` 与物理 `Ctrl`，统一标签格式化。registry 为 macOS 选择 `Primary+Shift+Z` 主绑定及 `Primary+Y` 别名，两者指向同一个 `edit.redo`；其他平台保留 `Primary+Y`。输入收集使用按键事件自身的修饰键，保留额外 Control / Command，避免同帧松键改变组合或把 Command+Tab 误当 Ctrl+Tab，同一组合每帧最多派发一次。

shell 的待新建、待打开、保存覆盖、关闭及单元删除确认优先于文本 / 画布焦点，文档快捷键不能穿透这些确认状态；文本 Undo / Redo 继续由 egui 编辑控件处理。命令、工程文件和求解模型语义未变。
...
- 本轮证据：30 项快捷键定向测试通过；全仓 1,147 项测试和严格 clippy 通过。macOS 普通空白来源 Feed–Heater–Flash 验证文本历史、树 / 画布 `⌘Z / ⇧⌘Z / ⌘Y`、revision 单增、结果旧化、新建确认隔离、`⌘S` 保存与跨进程重开 rerun。命令面板显示 `⌘S / ⌘Z / ⇧⌘Z`。沙盒端口限制、实窗修订和文件证据见 [W38](../devlogs/2026-09/2026-W38.md#2026-09-14-b1-4-平台快捷键与-b2-首轮复核)。
...
- 下一步：B1-4 已完成 macOS 收口；按 [B2-1 结果输出候选](results-review-diagnostics.md#b2-1轻量结果输出闭环) 明确下一实现。其他单元逐项 GUI、远距离视口与跨平台新增路径仍待验证。
