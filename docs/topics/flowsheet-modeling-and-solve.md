...
B0 表是旧基线记录，其中“删除缺失”已由本节覆盖，重命名仍缺失。下一切片建议优先定位已有对象却显示 `Canvas navigation anchor expired` 的反馈不一致：范围只含对象选择、视图锚点和编辑后反馈；验收为树 / 流股 / 单元导航可定位当前对象，拓扑变化后旧锚点正确重算，真正删除的对象仍给出明确反馈。先确认范围，不连带加入自由连接或通用 API。
