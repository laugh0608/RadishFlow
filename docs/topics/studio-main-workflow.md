...
B0 收口：受控 Feed–Flash 创建、参数提交、连接、目标端断开 / 重连、流股删除、命令面板撤销 / 重做和结果旧化已有实窗证据；当时单元删除 / 重命名缺失，macOS 首次保存缺失；后续 B1 已补齐删除、重命名和 macOS 项目选择器，并修正导航误报。完整 [编辑台账与 B1-1 实现](flowsheet-modeling-and-solve.md#b0-编辑能力复核2026-09-13)、[保存 / 重开分层结论](project-lifecycle-storage.md#b0-保存与重开复核2026-09-13) 与 [操作流水](../devlogs/2026-09/2026-W37.md#2026-09-13-b0-基础能力复核) 分别维护。其他单元逐项实窗、快捷键、导出和 Windows / Linux 本轮运行态仍待验证；B0 收口表示已建立差距与候选验收，不表示全部基础能力已通过。
...
- 快捷键复核：macOS 文本 / 文档历史边界、Enter 校验、显式丢弃、保存重开已补实窗证据；确认 Ctrl 标签与实际 ⌘ 不符、文档缺少 ⇧⌘Z 重做。Escape 保留草稿是现状，不等同取消缺陷。
- 下一步：平台快捷键展示与文档重做的 B1-4 候选，范围与验收见 [建模专题](flowsheet-modeling-and-solve.md#b1-4-候选平台快捷键展示与文档重做)；其他单元逐项实窗、远距离视口平移和 Windows / Linux 路径继续待验证。
