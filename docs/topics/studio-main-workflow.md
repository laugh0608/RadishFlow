...
B0 收口：受控 Feed–Flash 创建、参数提交、连接、目标端断开 / 重连、流股删除、命令面板撤销 / 重做和结果旧化已有实窗证据；当时单元删除 / 重命名缺失，macOS 首次保存缺失；后续 B1 已补齐删除和 macOS 项目选择器。完整 [编辑台账与 B1-1 实现](flowsheet-modeling-and-solve.md#b0-编辑能力复核2026-09-13)、[保存 / 重开分层结论](project-lifecycle-storage.md#b0-保存与重开复核2026-09-13) 与 [操作流水](../devlogs/2026-09/2026-W37.md#2026-09-13-b0-基础能力复核) 分别维护。其他单元逐项实窗、快捷键、导出和 Windows / Linux 本轮运行态仍待验证；B0 收口表示已建立差距与候选验收，不表示全部基础能力已通过。
...
- 下一步：确认 Canvas 导航锚点反馈修复范围；保留其他单元逐项实窗、快捷键和 Windows / Linux 新增路径的待验证标记，不提前建设自由连接或脚本框架。
