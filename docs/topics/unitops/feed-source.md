...
2026-09-16：Feed 的温压流量输入未来复用 [单位系统](../units-and-quantity-system.md)，核心 SI / 摩尔基准不变；缺参数和来源区分接入 [规格与辅助](../modeling-assistance-and-specifications.md)，不把模板默认值视为已提交规格。
...
- 下一步：完善 Feed 输入、组分与状态校验及诊断，保持建模和运行请求的一致性。
