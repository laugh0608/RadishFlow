...
- 已有 `light_text_export()`、复制和写文件实现，操作按钮位于 Runtime 区域；默认“结果”页尚未提供清楚的输出入口。`pick_native_result_export_file` 仅 Windows 接入，macOS / Linux 返回 `None`，上层将其当作用户取消，平台不支持与取消语义混淆。此为 2026-09-14 代码确认的缺口，不声称实际原生导出已验证。
...
- 当前快照的轻量文本复制 / 导出，B2-1 进入实现前确认具体契约。
...
下一切片建议围绕现有轻量输出完成普通用户闭环：

- 在默认结果页提供可发现的“复制 / 导出当前结果”入口，复用现有快照和文本序列化，不建设完整报告系统。
- current 可输出；stale / missing 明确不可作为当前结果输出；点击时再次检查快照身份及当前 revision，不导出旧页面持有的过时结果。
- 补 macOS 原生文本导出；Windows 保留并验证，Linux 在确定路径输入或原生选择器方案前明确显示不支持，不能将平台缺失伪装为用户取消。
- 区分成功、取消、不支持与写入失败；处理扩展名、覆盖确认和失败恢复，不损坏已有目标文件，不隐式修改工程或保存路径。
- 验收包括当前快照 ID / revision、单位和结果字段一致，未求解 / 编辑旧化后禁止输出，取消与失败不修改已有文件；先定向测试，再原生保存、文件读回和重开 rerun。

状态：范围建议已形成，尚未实施。具体命令归属、原子落盘复用与 UI 入口在开始本切片时确认；不把 Windows 历史实现视为 macOS / Linux 已交付。运行失败诊断恢复及其他单元逐项输出继续作为待补复核面。
...
- 下一步：按 B2-1 候选收口现有轻量结果输出的默认入口与平台文件选择 / 失败语义；运行失败诊断恢复继续复核，完整报表另行明确专题范围。
