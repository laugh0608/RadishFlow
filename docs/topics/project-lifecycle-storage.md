...
2026-09-16 规划新增 [单位偏好](units-and-quantity-system.md) 与 [设备任务](equipment-design-and-rating.md) 的保存恢复。纯显示偏好独立于求解输入修订，允许有单独保存需求；单位集 / 设备几何 / 工况来源的持久化所有权及迁移在实施切片确定，不修改旧 SI 字段解释，不提前写入占位 schema。设备候选与已采纳输入分开，恢复时重验模型 / 方法与源工况有效性；缺失能力不静默丢弃未知数据。
