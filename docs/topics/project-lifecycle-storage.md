...
2026-09-16 规划新增 [单位偏好](units-and-quantity-system.md#显示偏好所有权与兼容设计u2-待实现) 与 [设备任务](equipment-design-and-rating.md) 的保存恢复。U1 已明确单位集属于项目呈现配置，拟与 document 并列随工程保存，独立于求解输入修订及机器布局 sidecar；个人默认只初始化新工程，临时输入单位属于草稿。U2 再落实独立保存脏标记、DTO 与迁移，当前 schema 未改；旧工程保持完整 SI 解释，未知单位不得静默替换。设备几何 / 工况来源的持久化按各切片确定；候选与已采纳输入分开，恢复时重验模型 / 方法与源工况有效性，缺失能力不静默丢弃未知数据。

[工程基础与工况](engineering-basis-and-cases.md) 后续需要持久化共同条件、显式覆盖、来源、工程位号及案例身份；位号更名不改变内部引用。有效输入 / 方法变化与纯显示或说明变更分别管理，保存修订不自动等同求解输入修订。冻结运行依据与当前文档分离，来源更新不覆盖历史记录；具体 schema / 迁移及资产保存边界在 G1 / G3 和 Q2 / Q3 确定，旧工程来源缺失标为未记录，不伪造数据。
