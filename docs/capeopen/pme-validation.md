...
- 用户侧复验 no-op setter 与非空 placeholder 后，DWSIM 已继续进入 `ComponentName / ComponentDescription / Ports / Parameters`；COFE 仍在 `SimulationContext get` 返回后崩溃。下一轮复验应重点验证新增的 `ICapeSimulationContext` marker、`ICapeCOSEUtilities` named values 与 `ICapeDiagnostic` no-op logging 是否能让 COFE 继续推进到 identification / collections。
...
仍必须补齐的边界缺口不是新的 host round fallback，也不是继续盲补普通 optional interface。下一步先复验公开 simulation context placeholder coclass 与 `ICapeMaterialTemplateSystem` 修正；如果仍失败，再围绕 `DWSIM + COFE` 的 in-proc comhost 承载风险评估 out-of-proc COM shim / local server 或更保守的 PME in-proc shim + 外部 worker 策略。
