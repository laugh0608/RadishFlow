...
- `UnitOp.Mvp` 中最小 `IPersistStreamInit` 兼容面，用于让 PME 在把 Unit Operation 加入 flowsheet 画布或做持久化探测时能稳定取得 CLSID、初始化新对象并执行无状态 load/save/size 查询；当前不引入真实持久化，也不把 PME 持久化语义下沉到 Rust core
