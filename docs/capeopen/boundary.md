...
- 在 request plan 与单次 action execution 之上，当前又补出 `UnitOperationHostActionExecutionOrchestrator`，把“执行 ready requests 后宿主下一眼该看什么”继续收口为正式 result：统一返回 request plan、execution batch outcome 以及刷新后的 configuration/action plan/session；这层仍不负责 `Initialize / Validate / Calculate / Terminate`
