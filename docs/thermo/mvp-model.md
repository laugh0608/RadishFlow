...
- `tests/thermo-golden` 与 `tests/flash-golden` 当前也已补齐 near-boundary `±ΔP / ±ΔT` 小扰动金样；`rf-thermo` 与 `rf-flash` focused tests 会继续锁定 bubble/dew 两侧跨 boundary 前后的 phase region 与 `bubble_dew_window` 稳定行为
...
1. 把当前 `binary-hydrocarbon-lite-v1` 的 near-boundary `±ΔP / ±ΔT` 漂移监测扩到更多 overall composition / synthetic cases，避免边界样例仍只集中在单一二元基线
