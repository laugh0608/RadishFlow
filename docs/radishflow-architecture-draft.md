...
- 当前仍未进入 COM 注册或完整 PME 生命周期；不过最小 native 求解接线已打通，且 `Calculate()` 对外结果面当前已收口为稳定的“成功结果 + 失败摘要”双契约，并进一步提供统一只读查询面 `GetCalculationReport()` 及其上的最小文本导出面 `GetCalculationReportLines()/GetCalculationReportText()`，而不是继续直接暴露完整 snapshot JSON 或 native error JSON
...
- 当前可列出 package registry，并覆盖 direct adapter 的 flowsheet / stream snapshot JSON 导出，以及 `UnitOp.Mvp` 的最小成功结果契约、失败摘要契约、统一只读 report access 与最小文本导出面验证
