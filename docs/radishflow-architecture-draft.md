...
第一阶段仍只做稳态模拟闭环。以下方向进入 RadishFlow 的长期架构视野，但不进入当前 MVP、MVP β 或下一阶段执行范围：

- 动态模拟：未来可在稳态 flowsheet 与单元模型稳定后，扩展动态状态变量、时间积分器、控制 / 扰动场景、动态结果存储和回放 UI。它需要先有稳定的稳态求解、物性接口、项目 schema 版本治理和结果模型。
- CFD 模拟：未来可作为高保真局部流动、传热、多相流或设备内部场分析模块评估。它需要独立处理网格、边界条件、离散格式、求解器、后处理、算力管理和大数据结果存储，不应提前塞入当前 `rf-solver` 顺序模块法主线。
- 与稳态主线的关系：未来优先通过清晰的物性接口、单元边界、结果 DTO 和项目引用关系耦合，而不是让动态 / CFD 语义倒灌到当前 MVP 对象模型。
...
- 当前结果审阅继续通过 `StudioGuiWindowModel` 暴露只读 presentation：选中流股时可查看最新流股状态与组成，选中已运行单元时可查看最新 `SolveSnapshot` 中的执行状态、step 序号、summary、输入流股和产出流股跳转；求解步骤自身也携带单元、输入流股与产出流股 command action，供 Runtime、Active Inspector 和 Result Inspector 复用；失败摘要也携带 recovery action 与 recovery target action，继续复用运行栏修复命令和 Inspector target 命令；当前又把这些定位入口汇总成统一 `StudioGuiWindowDiagnosticTargetActionModel`，让失败摘要、Result Inspector、Active Inspector 与求解步骤消费同一类诊断目标 action；失败详情当前也会直接展示 latest diagnostic summary 中的 revision、severity、primary code、count 与相关 unit / stream / port target；Result Inspector 当前在 stream-centric 视图之外又补出并列的 unit-centric 视图，按单元维度展示最新 unit execution result、关联求解步骤、关联诊断和诊断目标动作，仍只复用既有 `inspector.focus_unit:*` / `inspector.focus_stream:*` 命令；这些都不回写项目文档，也不等同于结果导出、报表系统或新的错误处理状态机
