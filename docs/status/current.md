...
- **Studio UI 实现第十九刀已完成 focused 推进：`模块` 面板内部已按 `流股源 / 调节单元 / 汇合与分离` 组织现有受控 place-unit palette，并提供临时筛选；Feed、Heater、Cooler、Valve、Mixer、Flash Drum 仍通过正式 palette option 的 command id 派发，作者任务清单和画布建议保留在同一职责区；未新增完整模块库、自由连线、自动布线、完整拖拽布局或第二套命令状态。**
- **Studio UI 实现第二十刀已完成 focused 推进：`项目` 面板内部从线性列表整理为 `项目输入 / 示例入口 / 对象树 / 审阅状态` 四个职责区；物性包、项目组分、示例项目、流股 / 单元对象、结果和诊断仍消费既有 `workspace_document`、canvas object list、current snapshot / failure 状态，不新增完整项目浏览器、shell 私有项目状态、自由连线、自动布线或完整报表。**
- **Studio UI 实现第二十一刀已完成 focused 推进：中央 Canvas 已移除重复的可筛选对象列表，改为 `画布状态` 数量概览，并继续承接工具条、选择、视口、图例、画布实体和受控建议；对象树导航继续由左侧 `项目` 面板承担，不新增 shell 私有对象状态、完整项目浏览器、自由连线、自动布线或完整拖拽布局。**
- **Studio UI 实现第二十二刀已完成 focused 推进：右侧 `检查器 / 模块设置 / 模块结果` 增加同一 `画布选择` 上下文头，直接消费 Canvas current selection / command presentation；单元选择时三入口对齐到同一 active inspector target，流股选择时模块设置 / 模块结果保持 unit-only 空状态，不新增第二套对象状态、help command、完整参数表或右侧栏重排。**
- **Studio UI 实现第二十三刀已完成 focused 推进：中央 Canvas 首屏从独立 `选择 / 视口` 详情行收束为 `画布状态` 与 `画布操作` 两层；视口命令进入状态行，选中对象只保留聚焦、移动和连接恢复等可执行命令，选择详情继续由右侧 `画布选择` 承接，不新增第二套对象状态、自由连线、完整工具条或右侧栏重排。**
- **Studio UI 实现第二十四刀已完成 focused 推进：底部薄状态栏从完整 `状态汇总` 展开收束为运行 / 快照 / 单位 / 求解器 / 模式 / 当前选择扫读；完整 case、run、convergence、steps、diagnostics 与 snapshot 一致性继续由底部右侧 `状态汇总` 卡片承接，中央 Canvas、右侧选择上下文和底部运行状态的首屏职责已由 focused test 锁定。**
- **Studio UI 实现第二十五刀已完成 focused 推进：中央 Canvas 顶部工具区从重复的 `Place` 放置 palette 收束为 `画布工具`，只承接视图命令与当前建议命令；放置单元入口继续由左侧 `模块` 面板承担，`画布状态` 行保持纯状态 chip，不新增完整工具条体系、自由连线或第二套命令状态。**
- **Studio UI 实现第二十六刀已完成 focused 推进：Workbench 真实首屏职责完成一次 egui viewport 复核，顶部 `流程图工具栏` 的 Canvas 段只保留已有 suggestion / pending-edit 命令，不再重复左侧 `模块` 的放置 palette 或中央 `画布操作` 的选中对象移动命令；左侧、中央、右侧、底部抽屉和薄状态栏的主要职责位置已由 1280x860 focused test 锁定。**
- **Studio UI 实现第二十七刀已完成 focused 推进：基于真实 UI 反馈修正新建 / 返回导航；普通空白项目创建后先进入独立 `物性` 页，Home 的最近项目区会暴露当前 workspace 返回 tile，未保存项目不写入 recent projects，保存过的当前项目也不通过 recent path 重新打开自身。**
- **Studio UI 实现第二十八刀已完成 focused 推进：`物性` 页主路径与 `流程图` 建模交接已收口；`window.property_page` 从当前 `workspace_document` 派生 package / component 完成度和进入建模 readiness，`window.property_context_toolbar` 增加同源 `建模` 入口，顶部 `流程图` 导航、Property 页按钮和 Home 当前 workspace 返回入口共用同一进入 Workbench 行为。**
...
- 代码实现已允许从 presentation / window model 小切片推进，但仍不直接做大规模 egui 布局重排；Module Results DTO 已落到正式 window model 并由右侧 `模块结果` tab 消费，stale snapshot 不渲染旧 unit result；Home 最近项目继续由 shell preferences / recent path 持有，但已映射为同一 case tile DTO 并由 Home Dashboard 消费且人工复核通过；Home 还会从当前 `workspace_document` 派生当前 workspace 返回 tile，未保存项目不写入 recent projects；普通空白项目创建后先进入独立 `物性` 页，符合先选择物性包和项目组分再进入流程图建模的路径；`window.property_page` 现在从同一 `workspace_document` 派生进入流程图建模 readiness，Property 页入口、`window.property_context_toolbar` 的 `建模` 分组、顶部 `流程图` 导航和 Home 当前 workspace 返回入口共用这一判断；右侧 `模块设置` tab 已消费参数、端口和诊断，帮助入口当前仅表达“无正式 help command”；右侧栏已用同一 `画布选择` 上下文头消费 Canvas current selection / command presentation，让 `检查器`、`模块设置` 和 `模块结果` 与当前选择明确协同；独立物性页已接入顶部导航并消费既有 Property page DTO，`Package` 不再保留为右侧主入口；底部运行信息已补 `收敛 / 建议` 入口，并已进一步收敛为左侧运行信息 tabs + 右侧 `状态汇总` 分栏，底部薄状态栏只保留运行 / 快照 / 单位 / 求解器 / 模式 / 当前选择扫读，不再重复展开完整状态汇总；左侧栏已从 `项目 / 示例项目 / 放置` 收敛为 `模块 / 项目`，示例项目作为 `项目` 内的可展开入口保留；`模块` 面板内部已按现有受控单元类型分类并支持本地筛选，但不新增模块库真相源；`项目` 面板内部已按项目输入、示例入口、对象树和审阅状态分区，但不新增项目浏览器真相源；中央 Canvas 已收束为建模主舞台，首屏只保留 `画布工具`、`画布状态`、`画布操作`、图例、画布实体和受控建议，`画布工具` 只承接视图 / 建议命令，放置单元入口继续由左侧 `模块` 面板承担，选择详情和 viewport focus detail 由右侧栏 / 画布实体本身承接，不再重复渲染项目对象树、放置 palette 或独立选择详情行；顶部导航已按设计稿收敛为 `文件 / 主页 / 物性 / 流程图 / 运行 / 结果 / 工具 / 设置`，旧打开 / 保存 / 示例按钮不再作为 Workbench 主横排按钮常驻；`流程图` screen 的上下文工具栏已通过 `window.flowsheet_context_toolbar` 消费既有 Canvas suggestion / pending-edit command、Run command、Module Results / 结果表和状态 DTO，不再重复左侧放置 palette 或中央选中对象操作；`物性` screen 的上下文工具栏已通过 `window.property_context_toolbar` 消费既有 package / component command、property page 状态和进入建模 readiness；`运行` screen 的上下文工具栏已通过 `window.run_context_toolbar` 消费既有 Run Panel command / state、status summary、canvas suggestion count 和运行日志；`结果` screen 的上下文工具栏已通过 `window.result_context_toolbar` 消费既有 Module Results、结果表、Result focus command 和 `SolveSnapshot` 状态；`工具 / 设置` 菜单内容已拆成可测试分组并只消费 shell/window 既有状态。是否把 recent projects 从 shell preferences 上提到正式 snapshot 暂不作为当前 blocker。
