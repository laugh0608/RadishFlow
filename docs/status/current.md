...
- `apps/radishflow-studio::studio_gui_shell` 当前也已把 near-boundary `Result Inspector` selector state 补进正式回归：official `Feed/Heater/Cooler/Valve/Mixer -> Flash` 场景下，stream / comparison / unit 三类选择会继续沿同一份 `SolveSnapshot` consumer 语义切换；当 base stream 切到原 compared stream 时，comparison 会按既有 shell state 规则清空，重新选择对比流后又会恢复同一份 flash outlet comparison / unit view 结果，不在 selector 切换路径分叉第二套 consumer 状态。
- `apps/radishflow-studio::studio_gui_window_model` 当前也已把 near-boundary `inspector.focus_stream:*` / `inspector.focus_unit:*` 与 diagnostic action 补进正式回归：official `Feed/Heater/Cooler/Valve/Mixer -> Flash` 场景下，comparison focus action、flash unit view 的 consumed/produced stream action，以及 inlet / unit diagnostic action 都会继续指向同一份 flash inlet / outlet / unit consumer 目标；同一批回归也已通过临时 near-boundary project + `StudioGuiDriver -> run_panel.run_manual -> UiCommandRequested` 真 dispatch 前推到 Active Inspector，不在 action surface 和最终 active detail 之间分叉第二套 focus 语义。
- `apps/radishflow-studio::studio_gui_shell::tests::runtime` 当前也已把 near-boundary `diagnostic target / focus action` 收口到最终 runtime 渲染面：official `Feed/Heater/Cooler/Valve/Mixer -> Flash` 场景下，`Result Inspector / Active Inspector` 会继续正式回归 `DiagnosticTargets` section 的出现条件、flash outlet comparison 的 `Inspect` 标签，以及 shared diagnostic action renderer 的 `source | target | summary` 文本，不在 shell 最终显示层再分叉第二套 action surface 语义。
...
4. 若推进 Studio，优先继续消费已结构化 DTO 和既有 command surface，不新增第二套 shell 私有状态机；当前 near-boundary `SolveSnapshot -> window_model -> shell runtime` consumer/action 回归已基本闭环，下一步更值得先做阶段性仓库验证（`pwsh ./scripts/check-repo.ps1`），而不是继续沿同一条 UI action surface 做更细的局部打磨。
