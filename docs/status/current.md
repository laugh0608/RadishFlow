...
1. 继续围绕真实建模链路推进：首批单元参数保存 / 重开 / 运行、字段级约束提示、Valve 参数运行失败定位和连接类失败恢复路径已补齐；下一步可转向用户可见的最短建模能力补口，但仍不扩自由连线、自动布线或完整结果报表。
