...
2. B3-5 与 U1 已接通，下一步推进单位输入显示和现有输出，再完善现有单元的规格检查与确定性推荐；切片见 [路线图](../radishflow-mvp-roadmap.md#单位辅助设备与-ui-的实施顺序)。UI 信息架构与局部交互评审随切片衔接，纯领域基础不等待整套重绘。
...
- B3-3 受控创建 / 连接 / 运行和 B3-4 [首个无界面消费者](../reference/headless-cli.md) 已实现：共用事务、实时候选和诊断，支持查询、写入 SI 参数、同步求解与读取当前结果，保留原磁盘输入。
...
- U1 已实现 `rf-types::units` 静态量 / 单位目录与共享转换；变量元数据、浏览器及 CLI 标签接通，SI 协议保持。项目显示偏好归属已设计，U2 交互 / 存储待实现；验证见 W38。

### 下一步

- [ ] 明日（2026-09-17）优先 U2：先评审相关 R0 / R1 单位交互和存储兼容，再实现单位集、临时输入、草稿转换与保存恢复；偏好保存独立于求解修订。顺序与验收见 [明日事项](../devlogs/2026-09/2026-W38.md#明日事项2026-09-17)。
...
- 已提交单位 / 辅助 / 设备及工程基础规划、B3-5 和 U1；按今日代码核对快速开始、App 架构、存储、结果及 Studio 专题，修正过期描述。提交回顾与验证见 [W38 日终](../devlogs/2026-09/2026-W38.md#2026-09-16-日终回顾与明日事项)；P3 数值限制和新 UI 待评审状态保持。
...
- 2026-09-16：B3-5 / U1 通过 macOS 离线全仓基线，1,218 项测试与严格 clippy；含单位、变量元数据及 16 项 CLI 进程回归。使用进程级 Command Line Tools；本地 HTTP 测试经沙盒外复验通过，详见 W38。
