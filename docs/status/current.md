...
1. 若继续推进 `rf-thermo` / `rf-flash`，优先把当前已覆盖三组 two-phase overall composition 与 synthetic 单相样例的 near-boundary `±ΔP / ±ΔT` 漂移监测前推到 `tests/rust-integration` / workspace run path，继续锁定非 flash 中间流股进入后续 flash inlet 的端到端稳定性，而不是等到集成层或 UI 审阅才暴露。
