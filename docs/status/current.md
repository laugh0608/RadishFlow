...
- **Studio UI 实现第七刀已完成 focused 推进：右侧 Inspector / Module Settings 的参数、端口和诊断已确认来自正式 active Inspector state / command surface；`StudioGuiWindowModel` 新增 `module_settings` DTO，右侧 Inspector tab 对 active unit 窄口径消费该 DTO，不渲染 Module Results 的 latest-result 内容；帮助入口当前没有正式 command，DTO 显式暴露为空帮助状态。本轮未创建 `module-settings-panel.pen`。**
...
- 代码实现已允许从 presentation / window model 小切片推进，但仍不直接做大规模 egui 布局重排；Module Results DTO 已落到正式 window model 并由 Results tab 窄口径消费；Home 最近项目继续由 shell preferences / recent path 持有，但已映射为同一 case tile DTO 并由 Home Dashboard 消费且人工复核通过；右侧 Module Settings 已有窄口径 DTO 消费参数、端口和诊断，帮助入口当前仅表达“无正式 help command”；独立物性页第一刀已接入顶部导航并消费既有 Property page DTO，右侧 Package tab 暂留兼容入口。是否把 recent projects 从 shell preferences 上提到正式 snapshot 暂不作为当前 blocker。
