...
- 当前重点：首版 demo 前硬化期已经结束，当前进入“受控扩展高频建模能力”阶段。已收口 `Heater / Cooler / Valve` 参数链路、连接失败恢复、空白项目 Mixer 路径、受控流股断开 / 删除 / source-only 重连和原因提示、关闭脏工作区保护和 focused suggestion 下一步可见性。后续允许按正式边界推进下一批窄口径单元参数、受控重连细化、sidecar 级布局 / 视口体验和轻量结果审阅增强；仍不做自由连线编辑器、自动布线系统、完整拖拽布局编辑器、完整报表系统或第三方 CAPE-OPEN / 物性包加载。
...
- 2026-05-23 人工复核确认流股连接 / 断开交互已经顺滑；同日补关闭脏工作区的保存 / 舍弃 / 取消确认，并让 focused Canvas suggestion 在顶部摘要和图例中直接显示 `Connect stream` / `Create stream` 下一步动作，不改变 suggestion / `DocumentCommand` 边界。随后补选中 source-only 流股的受控重连入口：仅当当前流股已有唯一 source、没有 sink，且画布上只有一个可用 material inlet 时启用，写回仍走正式 `ConnectPorts` 与 undo history；不可用时说明缺 source、已有 sink、无 inlet 或候选不唯一。
