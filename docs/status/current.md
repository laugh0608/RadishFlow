...
- Mac 真实 UI 字体显示与普通空白项目 Heater 默认参数提交路径已由人工复核通过；Inspector 显示值与正式 unit parameter 的提交流程已对当前可编辑单元参数完成 focused 扫描。下一步继续观察普通空白项目真实建模缺口与 readiness / Run Panel 诊断边界：只在发现主路径 blocker、结果判断缺口或文档事实源冲突时继续推进。
