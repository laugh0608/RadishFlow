...
2. 若继续推进 `rf-thermo` / `rf-flash`，保持现有 golden 目录多样例遍历、focused tests 与 `Feed/Heater/Cooler/Valve/Mixer -> Flash` source/intermediate 到 consumer 的端到端一致性回归为同一套正式基线，不额外分叉第二套窗口估算、焓值求解或判断语义；若继续收口 fixture / provider 语义，优先清点剩余 generic helper 名称与示例项目命名是否还在混淆 official hydrocarbon 与 synthetic demo 族，而不是再让同一类 package/helper 同时承担两套组件目录。
