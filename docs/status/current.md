...
- `apps/radishflow-studio` 本地测试模块里残留的 crate-local `write_cached_package` helper 当前也已继续对齐到同一口径：`app_facade`、`run_panel_driver` 与 `workspace_control` 里的 official methane/ethane cache writer 现已统一改成显式 `write_official_binary_hydrocarbon_cached_package` 命名，不再在 Studio 自身测试层留下最后一批 generic helper。
- `apps/radishflow-studio::test_support` 共享 fixture/provider helper 当前也已继续收口：public surface 现只保留显式 official binary hydrocarbon provider / in-memory provider / cached-package writer；底层仍需 remap `package_id` / component ids 的 official sample-derived payload helper 已退回私有实现，`solver_bridge` / `workspace_solve_service` / runtime / window-model 测试不再直接依赖带 `component_specs` 的 `binary_hydrocarbon_lite_*` 泛称 API。
...
2. 若继续推进 `rf-thermo` / `rf-flash`，保持现有 golden 目录多样例遍历、focused tests 与 `Feed/Heater/Cooler/Valve/Mixer -> Flash` source/intermediate 到 consumer 的端到端一致性回归为同一套正式基线，不额外分叉第二套窗口估算、焓值求解或判断语义；若继续收口 fixture / provider 语义，当前可把重心从 official/synthetic 命名歧义转到重复 test fixture scaffolding 与 package-id literal 收敛，而不是再让 sample-derived helper 隐式暴露第二套身份语义。
