...
- **通用小流程建模 v1 第五切片已完成 focused 推进：普通空白项目组合路径结果核对已统一，host / window-model 层已锁定 blocked modeling input 不生成可恢复 Run Panel failure。**
...
- 已推进通用小流程建模 v1 第三至第五切片：`Feed -> Flash Drum`、`Feed -> Heater/Cooler/Valve -> Flash Drum` 与 `Feed + Feed -> Mixer -> Flash Drum` 均覆盖显式输入、保存端口绑定、重开 rerun、单元 step、结果检查器和 outlet T/P/F/H；host / window-model 层已补回归锁定 blocked modeling input 不启用 failure recovery，结构性连接 / 拓扑错误继续进入正式 Run Panel 诊断 / recovery。
