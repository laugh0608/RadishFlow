...
2. B3-5 后先推进单位元数据 / 转换、输入显示和现有输出，再完善现有单元的规格检查与确定性推荐；切片见 [路线图](../radishflow-mvp-roadmap.md#单位辅助设备与-ui-的实施顺序)。UI 信息架构与局部交互评审随切片衔接，纯领域基础不等待整套重绘。
...
- [ ] 优先推进 B3-5：将现有受控创建 / 候选连接接入无界面消费者，补齐创建返回身份的后续引用与逐步错误反馈。
- [ ] 先明确请求范围与兼容边界，再验证工程结构修改 → 参数写入 → 运行 → 结果读取；复用实时连接候选、修订检查和原有事务。B3-5 尚未实现或验收。
- [ ] 随后按 U1—U3、A1—A2 依次推进单位与规格基础；E0 / S0 / H0 明确首个设备工况与独立基准。规划已确认，具体代码接口、存储及依赖在切片开始前审定。
