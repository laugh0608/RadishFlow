...
- 已完成 B1-4 平台快捷键、B2-1 当前结果复制 / 安全导出、B2-2 失败恢复反馈、B2-3 Valve 与 B2-4 Mixer 用户闭环；macOS 所测路径通过，最新全仓基线 1,164 项测试与严格 clippy 通过。
- 日终前 6 个提交已逐一对照代码核查相关文档，日终只更新文档；提交回顾见 [W38 日终记录](../devlogs/2026-09/2026-W38.md#2026-09-14-日终回顾与明日事项)。

### 明日事项（2026-09-15）

- [ ] 优先 [B2-5 Cooler 空白建模与温压结果闭环](../topics/unitops/heater-cooler.md#b2-5候选cooler-空白建模与温压结果闭环)：先复核现有覆盖，再核对显式温压、无效草稿与上游联动、失败定位修复、保存重开重跑及当前结果输出；只对真实缺口补实现与回归。
- [ ] 完成 macOS 普通空白项目实窗、文件读回和与修改范围相称的验证，更新专题与周志；未测的平台保持待验证。
- 后续按需补其余单元、远距离视口和 Windows / Linux 原生键盘 / 保存路径。真实窗口前先告知，UTM 串行且最多一台运行。
- 插件运行时、具体许可政策、公共 API 与完整报告继续按独立规划推进，不并入明日基础功能切片。
