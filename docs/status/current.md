...
- **Studio UI 实现第五刀已完成 focused 推进但仍不做 UI 大改：Results tab 已窄口径消费 `StudioGuiWindowModel::module_results`，当前选中单元可展示 selected unit result、consumed / produced stream chips 与 stream focus actions；文档编辑后的 stale module results 只显示过期提示，不渲染旧 unit result。**
...
- 代码实现已允许从 presentation / window model 小切片推进，但仍不直接做大规模 egui 布局重排；Module Results DTO 已落到正式 window model 并由 Results tab 窄口径消费，后续再决定是否拆出专门模块结果页或只保留当前入口；Home 最近项目来源尚未上提到正式 snapshot，后续需要决定是否纳入同一 tile DTO。
