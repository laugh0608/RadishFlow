...
- [ ] 先明确请求范围与兼容边界，再验证工程结构修改 → 参数写入 → 运行 → 结果读取；复用实时连接候选、修订检查和原有事务。今晚仅登记，尚未实现或验收。
...
- 2026-09-15：B2-5、B3-1 至 B3-4 分别通过 macOS 离线全仓基线，最终 1,199 项测试与严格 clippy 通过；Cooler 和变量浏览有实窗证据，B3-4 有 8 项真实 CLI 进程回归。其他平台原生未测，详见 [W38](../devlogs/2026-09/2026-W38.md)。
