...
2026-09-13 首轮 B0 已完成代码、定向测试与 macOS 普通空白项目实窗复核。受控编辑与流股操作已有证据；单元删除 / 重命名、macOS 原生首次保存仍缺失，未测平台和路径不记为缺陷。下一步确认 [B1-1 单元删除与关联清理候选](../topics/flowsheet-modeling-and-solve.md#b1-1-候选单元删除与关联清理待确认未实施)；[macOS 文件选择器](../topics/project-lifecycle-storage.md#b0-保存与重开复核2026-09-13) 单独处理，B1 尚未实施。账户联合登录仍为待架构决策的 Draft。
