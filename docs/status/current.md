...
1. 若继续推进 `rf-thermo` / `rf-flash`，优先把当前 two-phase 与 synthetic 单相样例都已前推到 `tests/rust-integration` / workspace run path 的 near-boundary `±ΔP / ±ΔT` 基线维持为正式回归；若要继续扩展，优先沿同一模式补到 `Mixer`、`Cooler`、`Valve` 等更多非 flash 中间流股链路，而不是分叉第二套判断语义。
