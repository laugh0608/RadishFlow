...
2026-09-13 已完成 B0 分层复核、B1-1 单元删除、macOS 原生项目保存、B1-2 导航修复和 B1-3 重命名。六类事务测试及 macOS 普通空白来源的 Feed–Heater–Flash 实窗主路径通过；其他单元逐项实窗及 Windows / Linux 新增路径仍待验证。2026-09-14 B1-4 已修复平台标签与 ⇧⌘Z 文档重做，并通过 macOS 焦点隔离、单次事务和保存重开验收；具体状态与验收见 [建模专题](../topics/flowsheet-modeling-and-solve.md)，保存边界见 [生命周期专题](../topics/project-lifecycle-storage.md)。账户联合登录仍为待架构决策的 Draft。

### 当前待办（2026-09-14）

- [x] [B1-4 平台快捷键展示与文档重做](../topics/flowsheet-modeling-and-solve.md#b1-4平台快捷键展示与文档重做)：统一 Primary / 物理 Ctrl、macOS ⇧⌘Z 主键与 ⌘Y 别名；文本 / 文档历史、确认状态隔离、单次事务、旧化与保存重开通过 macOS 验收。
- [x] B2 首轮复核：空白物性配置门禁、非法温度拒绝、重开 rerun 和轻量结果审阅通过所测 macOS 路径；已确认结果输出默认入口与非 Windows 导出缺口。
- [x] [B2-1 轻量结果输出闭环](../topics/results-review-diagnostics.md#b2-1轻量结果输出闭环)：默认结果页复制 / 导出、执行时重新校验 current、macOS 原生选择器与安全覆盖完成；取消 / 不支持 / 写入失败分别反馈，macOS 实窗及全仓验证通过。
- [x] [B2-2 运行失败诊断与恢复](../topics/results-review-diagnostics.md#b2-2运行失败诊断与恢复)：补齐顶部恢复入口、恢复后的画布定位、F5 / F8 面板一致性；参数 / 连接失败修复与保存重开实窗通过。
- [x] [B2-3 Valve 空白建模与参数联动](../topics/unitops/valve.md#b2-3空白建模与参数联动)：修复保留草稿随入口压力变化的校验，macOS 空白建模、保存重开重跑及当前结果输出通过。
- [ ] 下一优先项：Mixer 双进料建模、参数联动和结果复核，随后 Cooler。完整报告和公共 API 不并入。
- [ ] 按需补其余单元、远距离视口和 Windows / Linux 原生键盘 / 保存路径；未测项保持待验证。真实窗口前先告知，UTM 串行且最多一台运行。
...
- 2026-09-14：B2-1 / B2-2 分别完成轻量结果输出和失败恢复，macOS 全仓及对应实窗通过，详见 [W38](../devlogs/2026-09/2026-W38.md)。
- 2026-09-14：B2-3 通过 macOS 真实环境离线 `check-repo`（1,162 项测试、严格 clippy），Valve 空白建模、压力草稿联动及保存重跑 / 导出实窗通过；Windows / Linux 原生路径未复验，见 [记录](../devlogs/2026-09/2026-W38.md#2026-09-14-b2-3-valve-空白建模与参数联动)。
