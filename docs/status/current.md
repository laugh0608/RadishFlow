...
- [ ] 下一优先项：Valve 普通空白建模、参数联动、保存重跑及结果输出复核；随后覆盖 Mixer / Cooler。完整报告和公共 API 不并入。
...
- 2026-09-14：B2-1 通过 macOS 真实环境离线 `check-repo`（1,154 项测试及严格 clippy），原生结果导出、取消 / 覆盖和 current / stale / missing 实窗通过；Windows / Linux 原生路径未复验，详见 [W38](../devlogs/2026-09/2026-W38.md#2026-09-14-b2-1-轻量结果输出闭环)。

- 2026-09-14：B2-2 通过 macOS 真实环境离线 `check-repo`（1,159 项测试、严格 clippy），参数 / 连接失败恢复和 F5 保存重开实窗通过，见 [W38](../devlogs/2026-09/2026-W38.md#2026-09-14-b2-2-运行失败诊断与恢复)。
