...
- 2026-05-23 人工复核确认流股连接 / 断开交互已经顺滑；同日补关闭脏工作区的保存 / 舍弃 / 取消确认，并让 focused Canvas suggestion 在顶部摘要和图例中直接显示 `Connect stream` / `Create stream` 下一步动作，不改变 suggestion / `DocumentCommand` 边界。
