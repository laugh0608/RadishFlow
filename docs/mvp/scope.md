...
动态模拟、CFD、第三方 CAPE-OPEN 模型加载、完整 Thermodynamics PMC 等不属于当前 MVP 范围。新增模拟能力、物性模型、控制面或发布能力按用户需求、前置条件和专题验收逐步安排。

下一阶段先选择目标用户、体系和工况，再确定具体范围与验收；持续处理已有可靠性问题。迭代顺序见 [路线图](../radishflow-mvp-roadmap.md#后续迭代顺序)，具体切片由当前状态统一管理。
