...
- 当前剩余重点已经进一步转向真实 HTTP client adapter 接线、联网失败策略细化、数值主线和求解闭环，而不是继续停留在第一轮 DTO 草案
...
- 在已接通的下载 HTTP transport 端口之上补真实 HTTP client adapter，把实际客户端异常映射到既有 transport error 与失败分类
