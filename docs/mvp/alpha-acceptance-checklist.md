...
> 本文保留历史验收标准与当时记录，`Pass` 只说明对应提交和场景通过，不自动外推到当前 Windows / GUI 状态或物理准确性。当前开发与验证安排见 [当前状态](../status/current.md)，数值证据边界见 [热力学模型](../thermo/mvp-model.md)。
...
截至 2026-05-25，MVP α 用户视角 smoke 和首版 demo 前硬化期已经阶段性收口。本文后续作为验收记录和回归参考保留；当时后续主线转向 MVP β 高频建模与小案例作者体验；当前迭代安排以 [当前状态](../status/current.md) 为准。历史 `v26.5.1-dev` 材料只作为 staging 草案保留，不再作为正式 tag / release 节点事实源。
