...
截至 2026-05-25，MVP α 用户视角 smoke 和首版 demo 前硬化期已经阶段性收口。本文后续作为验收记录和回归参考保留；当时后续主线转向 MVP β 高频建模与小案例作者体验；该历史切换不表示当前恢复业务开发。历史 `v26.5.1-dev` 材料只作为 staging 草案保留，不再作为正式 tag / release 节点事实源。
