...
- `UnitOperationPortPlaceholder.Connect` 当前已改为只读取 material object 的 `ICapeIdentification` 快照，并在 finally 中 `FinalReleaseComObject` PME 传入的 COM 入参；用户侧 trace 已确认 `released-com-argument`，该路径暂不再作为 COFE 当前 blocker
...
- 下一轮应配置 `Flowsheet Json`、property package id 与必要 companion 路径后，继续复验完整 `Validate / Calculate / Report` 成功路径；当前缺参数导致的 `Validate` invalid 是预期行为
