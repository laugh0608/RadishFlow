...
- Studio 当前窗口布局状态已覆盖 `panel dock_region/visibility/collapsed/order`、region 内 stack placement、`center_area`、`region_weights` 与多窗口 `layout scope`
...
- Studio 当前也已继续把布局变更事件正式收口到 `WindowLayoutMutationRequested -> WindowLayoutUpdated(...)`，覆盖 `panel dock_region/visibility/collapsed/order`、region 内 stack placement、`center_area` 与 `region_weights`
...
- 在已形成的 `StudioGuiWindowLayoutState` 基础上，继续补真实 dock 编排契约，例如拖拽预览、panel 分组堆叠、跨窗口布局模板与更完整的标题栏/窗口 scope 语义
