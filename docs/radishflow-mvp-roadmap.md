...
- Studio 当前已把最小真实桌面闭环接到 `egui` Runtime 面板：用户可在内置正向示例项目之间切换，触发既有运行栏动作，并查看 `SolveSnapshot` 映射出的流股结果、求解步骤、诊断、日志与工作区摘要。
...
- 这一路径仍限定在“打开已有项目或内置示例 -> 最近项目入口 -> 运行求解 -> 查看结果/诊断”的最小可操作工作台闭环，不代表已经进入完整画布编辑器、原生文件选择器、保存/另存为、结果表格导出、最近项目持久化或 UI 视觉精修阶段。
...
同日 Studio 主线已补出第一版可见闭环：内置示例切换、运行求解、结果/诊断查看、中文 shell 选项与系统 CJK 字体 fallback 已进入真实 `egui` 壳层；2026-04-28 又补出路径输入式项目打开入口、打开成功/失败反馈、未保存改动打开前确认、shell 级最近项目列表与结果区基础本地化。下一步应继续收敛原生文件选择器评估、更完整结果 inspector 与最近项目持久化策略，而不是回到 CAPE-OPEN 兼容扩张。
