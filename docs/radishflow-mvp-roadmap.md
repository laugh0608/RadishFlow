...
- `Registration` 当前已继续前推到真实 `TypeLib` 注册链路：dry-run 会解析 `ResolvedTypeLibraryPath`、校验 `TypeLib GUID/version`，execute 会按 scope 调用 `RegisterTypeLib(ForUser)` / `UnRegisterTypeLib(ForUser)`，并把 `TypeLib` 纳入备份/回滚范围
...
- 优先用新的 execute 路径在真实环境完成一次 `current-user` `TypeLib` 注册复验，并先让 PowerShell 晚绑定 COM 探测通过，再回到 `DWSIM + COFE`
