...
- Studio 当前又已补出 `Save / Save As` 文档生命周期、字段编辑快捷键焦点策略、项目文件 staged write 和 `Save As` 覆盖确认：保存只写回项目真相源并刷新保存态，不进入文档历史；`Ctrl+S` 在文本输入焦点下仍保存，`Ctrl+Z / Ctrl+Y` 在文本输入焦点下保留给输入框；覆盖已有非当前项目文件前必须先停在 shell 确认态。
- 这一路径仍限定在“打开已有项目或内置示例 -> 最近项目入口 -> 运行求解 -> 查看结构化结果/诊断 -> 定位 Inspector -> 编辑 Stream 字段草稿并提交 -> 基础撤销/重做 -> 保存/另存为 -> 快捷键与覆盖确认保护”的最小可操作工作台闭环；当前原生文件选择器只覆盖 Windows 打开与另存为，不代表已经进入完整画布编辑器、跨平台文件工作流、结果表格导出、完整应用偏好系统、跨会话历史持久化或 UI 视觉精修阶段。
