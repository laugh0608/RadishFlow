...
- Studio 当前又已补出 Runtime 面板内的项目路径打开入口：用户可输入现有 `*.rfproj.json` 路径并重建当前 Studio runtime；打开失败时保留当前工作区并显示错误反馈，内置示例切换复用同一条打开流程；若当前文档存在未保存修订，打开动作会先进入显式确认状态。
- Studio 当前中文/英文切换先作为 GUI shell 级偏好存在，默认显示中文壳层文案；中文字体通过系统 CJK 字体 fallback 解决，不把字体资产或语言偏好写入 `*.rfproj.json`。
- 这一路径仍限定在“打开已有项目或内置示例 -> 运行求解 -> 查看结果/诊断”的最小可操作工作台闭环，不代表已经进入完整画布编辑器、原生文件选择器、保存/另存为、结果表格导出或 UI 视觉精修阶段。
...
同日 Studio 主线已补出第一版可见闭环：内置示例切换、运行求解、结果/诊断查看、中文 shell 选项与系统 CJK 字体 fallback 已进入真实 `egui` 壳层；2026-04-28 又补出路径输入式项目打开入口、打开成功/失败反馈、未保存改动打开前确认与结果区基础本地化。下一步应继续收敛最近项目列表、原生文件选择器评估与更完整结果 inspector，而不是回到 CAPE-OPEN 兼容扩张。
