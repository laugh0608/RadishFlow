...
- 因此 M5 后续不应继续把主要精力放在盲补普通 COM/OLE optional interface；需要先评估 out-of-proc COM shim / local server 或 in-proc shim + 外部 worker，把 `.NET 10` runtime 从目标 PME 进程内隔离

基于上述进展，明天优先计划进一步对齐为：

- 优先评估 `DWSIM + COFE` 下继续使用 `.NET 10 in-proc comhost` 的可行性；若目标仍是这两个 PME，下一步更可能是 out-of-proc COM 承载方案，而不是继续追加普通 optional interface
