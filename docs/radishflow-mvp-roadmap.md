...
- 当前 PME-like 薄宿主入口与 registration preflight 已足以证明正式消费路径和组件身份口径可继续承接 M5；`Registration` 当前已能 dry-run 生成 `register / unregister` 与 `current-user / local-machine` 的 registry key plan。下一步若继续推进，应转向执行型 COM 注册工具的安全边界、真实写入前置检查、安装/反安装脚本、以及目标 PME 人工验证说明，而不是继续深挖 host round 兜底或 smoke driver 抽取
