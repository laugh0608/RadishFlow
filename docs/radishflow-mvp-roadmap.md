...
同日后续真实 `DWSIM / COFE` dump 采集显示，当前阻塞点已超出普通 TypeLib / optional interface 补齐范畴：
...
- 因此 M5 后续不应继续把主要精力放在盲补普通 COM/OLE optional interface；先复验 raw pointer persistence 修正。若仍失败，再评估 out-of-proc COM shim / local server 或 in-proc shim + 外部 worker，把 `.NET 10` runtime 从目标 PME 进程内隔离

同日晚间继续沿真实 `DWSIM / COFE` trace 收敛，当前进一步结论如下：

- `ICapeUtilities.SimulationContext` setter 必须保持 raw `IntPtr`，避免 DWSIM 在进入 setter 前触发 CLR interface marshaler；getter 则必须保留 `[return: MarshalAs(UnmanagedType.IDispatch)]`，这是 COFE 越过 `SimulationContext get-exit` 的必要形状
- `COFE` 当前已一度推进到可放置 unit、连接 inlet/outlet material streams 并调用 `Initialize()`；关闭 case 时的 material object release warning 当前优先按 UnitOp.Mvp 是否仍持有 PME material COM RCW 排查
- `UnitOperationPortPlaceholder.Connect` 当前已改为只读取 material object 的 `ICapeIdentification` 快照，并在 finally 中 `FinalReleaseComObject` PME 传入的 COM 入参；下一轮应重点确认 trace 是否出现 `released-com-argument`
- `DWSIM` 当前不再硬崩，已能推进到 `SimulationContext get`、`ComponentName / ComponentDescription set`、`Ports get` 与 `Parameters get`，但仍不能落到画布
- 注册计划当前已补齐 `Consumes Thermodynamics` 与 `Supports Thermodynamics 1.1` 两个 CAPE-OPEN implemented categories，作为 DWSIM 画布接受条件 probe；这不代表 MVP 已实现 Thermo PMC 或第三方 property package 加载
- `ICapeCollection.Item / Count` 当前已加入 trace；下一轮 DWSIM 复验应优先判断是否出现 `Ports.ICapeCollection.Count/Item` 与 `Parameters.ICapeCollection.Count/Item`

基于上述进展，明天优先计划进一步对齐为：

- 先恢复到“COFE 能越过 `SimulationContext get-exit`”这一已验证状态，再继续看 material object release warning 是否被 COM 入参释放修正消除
- 对 DWSIM，优先根据新增 collection trace 判断它是否实际消费 `Ports / Parameters` 集合内容；若没有，应围绕 collection QI/typeinfo、implemented categories 与 OLE canvas 接受条件继续定向 probe
- `current-user` 下的标准 `TypeLib` 注册方案当前已落到代码；若必须评估 `local-machine`，后续只作为次选策略单独判断
- 不再把问题粗略归为 `TypeLib` 注册缺失，也不再继续盲补 COFE simulation context optional interfaces
