...
- Studio 当前窗口布局状态已覆盖 `panel dock_region/stack_group/visibility/collapsed/order`、region 内 stack placement、`center_area`、`region_weights` 与多窗口 `layout scope`
...
- 在已形成的 `StudioGuiWindowLayoutState` 基础上，继续补真实 dock 编排契约，例如 tabbed group 标题/激活态、拖拽预览、跨窗口布局模板与更完整的标题栏/窗口 scope 语义
