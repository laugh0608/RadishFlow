...
- `rf-ui` 当前已不只拥有运行意图类型，还拥有最小按钮模型、文本渲染 DTO 与 `RunPanelPresentation`；后续真正视图层接线时不必再重新发明运行栏启用判断、按钮槽位组织或文本布局
- 当前最小桌面入口 `run_studio_bootstrap` / `main.rs` 已开始直接消费 `RunPanelPresentation`，作为真正的最小运行栏组件/消费层
