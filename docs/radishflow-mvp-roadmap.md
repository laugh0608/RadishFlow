...
- 优先把 raw pointer persistence 修正带回 `DWSIM + COFE` 人工复验；若 trace 仍停在同一阶段，再评估继续使用 `.NET 10 in-proc comhost` 的可行性和 out-of-proc COM 承载方案
- `current-user` 下的标准 `TypeLib` 注册方案当前已落到代码；若必须评估 `local-machine`，后续只作为次选策略单独判断
- 若 `DWSIM / COFE` 仍失败，优先按 `ICapeUnit`、collection、port、parameter 或 report 的实际调用点分类记录，不再把问题粗略归为 `TypeLib` 注册缺失
- 在 `DWSIM + COFE` 未复验前，不再把主要精力放回 `Aspen Plus` 或 GUI 层重复点击验证
