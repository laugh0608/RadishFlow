...
- `RadishFlow.CapeOpen.UnitOp.Mvp` 当前又已继续补出 `GetCalculationReportDetailValue(string)`，作为最小 detail 键值消费面，让宿主侧可直接读取 `status`、`highestSeverity`、`diagnosticCount`、`requestedOperation`、`nativeStatus` 等稳定 detail key
