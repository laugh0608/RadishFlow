...
- 把已形成的 host-facing 只读模型、request planning 模型、host view snapshot，以及 orchestration / validation / calculation outcome + follow-up 模型视为正式库内消费面，未来 smoke host / PME host / 其他宿主入口都应优先复用，而不是各自重组配置、动作、输入规划、执行后刷新、material、execution 与 session 语义
