...
用途：记录第一阶段成果、后续迭代顺序与验收依据，并提供历史规划入口。
读者：需要理解现有成果、历史目标和可能的投资顺序的项目所有者与维护者。
不包含：当前任务清单、逐日验证流水、完整 UI 设计和新的产品交付承诺。

## 维护状态

自 2026-09-12 起恢复正常开发和迭代，以已有 MVP 为基础推进数值可信度、项目可靠性与用户主路径。当前优先级与下一切片以 [当前状态](status/current.md) 为准；旧 Sprint 保留为历史记录。
...
以下顺序承接现有实现与 2026-09-06 审阅结果，用于正常迭代的选题与验收。已知数据恢复和内存安全风险可先行复现与修复；新数值模型的选择以目标工况和独立验证依据为前提。
