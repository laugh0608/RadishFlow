...
截至 2026-04-23，M5 当前又进一步收敛到 `TypeLib` 主线，新增结论如下：
...
- `UnitOp.Mvp` 当前已补入冻结的 `typelib/RadishFlow.CapeOpen.UnitOp.Mvp.idl`、首份 `typelib/RadishFlow.CapeOpen.UnitOp.Mvp.tlb`，并已验证 `<ComHostTypeLibrary ...>` 可接入 `.NET comhost` 构建；但单靠嵌入 `TLB` 仍不能替代标准 `TypeLib` 注册
- 本机工具链当前已确认可用：`D:\Windows Kits\10` 下的 `midl.exe / rc.exe` 与现有 Visual Studio `cl.exe` 已可用于继续推进 `IDL -> TLB`；问题不再是“本机没有工具”

基于上述进展，明天优先计划进一步对齐为：

- 优先补齐 `IDL -> TLB -> TypeLib` 生成与注册链路，目标先让 PowerShell 晚绑定 COM 探测通过，再回到 `DWSIM + COFE`
- 优先确认 `current-user` 下是否存在可接受的标准 `TypeLib` 注册方案；若必须评估 `local-machine`，也只作为次选策略单独判断
- 额外检查 `D:\Program Files\Microsoft Visual Studio\18\Insiders` 下 VS 2026 Insiders 工具链是否可用于 `midl/rc/cl` 组合调用，并决定后续脚本的首选探测顺序
- 在上述链路未打通前，不再把主要精力放回 `Aspen Plus` 或 GUI 层重复点击验证
