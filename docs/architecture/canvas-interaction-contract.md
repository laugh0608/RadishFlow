...
- 普通空白项目的运行 gate 属于 App 层通用 `Flowsheet` readiness：它按 material port / stream reference、Feed source stream T/P/F/z、composition 引用 / 归一和必要单元参数判断，并聚焦到 stream / unit / port。画布 suggestion 只帮助创建拓扑和 outlet stream，不代表输入已经完整。
...
- 用户触发 `Fit to content` 时，只允许基于当前 Canvas 内容 bounds 重算并保存 offset；不保存缩放、滚动历史、选区、完整 camera state，也不改变单元 placement。
