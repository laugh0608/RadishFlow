...
- Studio shell 中央 Canvas 只承担建模主舞台职责，显示 `画布状态` 概览、工具条、选择、视口、图例、画布实体和受控建议；项目对象树和对象过滤导航由左侧 `项目` 面板承担，中央 Canvas 不渲染第二份对象树。
...
- 非空画布不应常驻空状态提示、开发态计数摘要或对象树副本；画布 header 只保留用户理解当前状态所需的短标签、legend 和可行动工具。
