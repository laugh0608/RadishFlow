...
P0 先行。P1 不再维护完整 Workbench 复制稿；只有当 `studio-client-main.pen` 中的模块设置 / 模块结果结构评审后仍不够细，再创建窄口径 `module-settings-panel.pen`。当前右侧栏第一轮实现已收敛为 `检查器 / 模块设置 / 模块结果`：模块设置的参数摘要、参数字段、端口和诊断来自正式 active Inspector state / command 来源；模块结果来自 `window.module_results`，current 结果态采用紧凑单元、状态、step、短执行摘要和消费 / 产出流股 chip，不在首屏展开长 snapshot id 或完整执行句；帮助入口尚无正式 command，因此暂不创建 `module-settings-panel.pen`。左侧栏已收敛为 `模块 / 项目`：`模块` 承接放置 palette、作者任务清单和画布建议，并已把现有受控单元按 `流股源 / 调节单元 / 汇合与分离` 分类和本地筛选组织；分类和选项 detail 保留在 hover / DTO，不作为首屏常驻说明；`项目` 承接项目输入、示例入口、对象树和审阅状态，继续消费项目对象、项目级输入扫读和可展开示例入口；official hydrocarbon 示例项目文件持久化 `binary-hydrocarbon-lite-v1`，项目输入、独立物性页和 Home case tile 都从当前 document / builtin choice 状态读取 package，并展示可读 label，稳定 id 只留在项目文件、command id、运行请求和内部状态边界。中央 Canvas 已在左侧分区稳定后收束为建模主舞台：它只渲染 `画布状态` 数量概览、工具条、选择、视口、图例、画布实体和受控建议，不再重复渲染项目对象树。底部运行信息区域已补齐 `收敛 / 建议` 入口，并已完成左右分栏第一刀：左侧继续承接 `消息 / 运行日志 / 收敛 / 建议 / 诊断 / 结果表`，右侧固定消费 `window.status_summary` 展示 case、run、convergence、steps、diagnostics 与 snapshot 一致性；snapshot 一致性已提升到状态汇总标题行扫读，完整收敛曲线、完整建议系统和完整报表仍不进入范围。`物性` 页到 `流程图` 建模的主路径交接已由 `StudioGuiWindowPropertyPageModel` 从当前 `workspace_document` 派生 package / component 完成度，Property 页入口、`window.property_context_toolbar`、顶部 `流程图` 导航、Home 显式 `返回工作区` 入口和内部进入 Workbench 行为共用同一 readiness；当前 MVP 的 `物性` 页不渲染无操作价值的左侧二级导航，只保留受控选择区与摘要区；entitlement host schedule notice 只输出用户可读授权检查摘要，不把 timer event 或 `SystemTime` 调度结构作为主界面文案；普通空白项目进入空 Workbench 后的首屏区域职责已用 focused test 锁定。P2 只做边界和页面职责，不进入完整设计稿细化，除非 P0 / P1 已经评审通过。

当前不把端点拆到更细，例如单独的 Home 设计稿、Canvas 设计稿或 Result 设计稿。原因是 Studio 客户端本体必须先统一主工作流和区域职责，再决定是否拆分子稿。若后续 P0 设计稿过大，再按 Home / Property / Workbench / Result 等域拆分，并保留同一命名前缀。独立物性页当前已完成顶部导航下的实现切片，继续消费正式 Property page DTO；`Package` 已不再作为右侧栏主入口，物性主路径由独立 `物性` screen 承担，进入流程图建模的状态只从 package / 项目组分完成度派生，内部进入 Workbench 入口也不绕过该 readiness；Property toolbar、摘要和 package 选择卡展示 selected builtin label，不把 raw package id 当作主展示文本；当前 egui MVP 不把 `参数 / 分析 / 来源` 等未来空间渲染成禁用二级导航。顶部导航已从旧 `快速操作` 横排按钮收敛到 `文件 / 主页 / 物性 / 流程图 / 运行 / 结果 / 工具 / 设置`：打开 / 新建 / 保存 / 示例归入 `文件`，运行和结果只路由到现有 run command、右侧 `模块结果` 和底部结果表，不新增完整 ribbon 或第二套命令状态。`流程图` screen 已完成上下文工具栏第一刀：`window.flowsheet_context_toolbar` 只从既有 command registry、canvas presentation、Run Panel / status summary、Module Results 和 current / stale / missing snapshot 状态派生，Canvas / Run 只渲染已启用命令，Review 入口只指向右侧 `模块结果` 与底部 `结果表`。普通空白项目进入空 Workbench 后，左侧 `模块`、中央 `画布工具 / 画布状态`、右侧 `画布选择 / 检查器` 与底部消息 / 状态汇总的首屏职责已由 focused test 锁定。`物性` screen 的上下文工具栏已从 `StudioGuiWindowPropertyPageModel` 派生：package / component command id、selected / remove-enabled 状态、内置来源摘要和进入建模 readiness 均来自当前项目文档，不引入第二套物性状态源或尚未进入范围的物性能力。`运行` screen 已完成上下文工具栏职责收束：`window.run_context_toolbar` 只从 Run Panel command registry、Run Panel state、status summary、canvas suggestion count 和运行日志派生 Control / Recovery / Monitor 入口；Monitor 入口在顶栏不重复展示状态 chip，状态继续由 status items 和底部状态汇总扫读，不把运行入口扩成完整控制台、完整日志系统、批量运行、自动调度或完整报表。`结果` screen 已完成上下文工具栏职责收束：`window.result_context_toolbar` 只从 Module Results、底部结果表、当前 / stale / missing `SolveSnapshot` 状态和已启用 Result focus command 派生 Review / Focus / Status 入口；顶栏不展开所有 focus command，具体对象定位由结果表、右侧相关 action、项目树和命令面板承担，不扩完整报表、跨快照报表、模板、打印或批量导出。`工具 / 设置` 顶层菜单已完成职责对齐第一刀：工具菜单只组织命令面板、Commands panel 可见性和逻辑窗口，设置菜单只组织当前语言选择；help command、插件管理、单位集设置、完整偏好页、账号 / 授权 / 服务器设置和发布入口仍不进入当前实现。
...
| 区域 | 职责 | 不承担 |
| --- | --- | --- |
| Home | 开始项目、返回当前工作区、打开示例、最近项目、环境状态、登录入口；最近 / 示例案例使用 RadishFlow 浅色流程缩影 tile gallery 展示项目名、路径 / 来源、时间和状态 | 营销页、完整控制台、完整命令面板、HYSYS 深蓝文件图标或左侧文件菜单复刻 |
| Property Page | 独立物性页：当前 MVP 承接受控 package / 项目组分选择、摘要和进入建模 readiness；长期可扩组分、方法、参数、来源、分析入口 | 完整组分数据库、第三方物性包加载、完整 Thermodynamics PMC |
...
3. 从同一 Property DTO readiness 进入 `流程图` 建模，空 flowsheet 聚焦左侧 `模块` 放置入口。
4. 放置 `Feed -> Flash Drum`，补齐输入，运行并查看结果。
5. 放置 `Feed -> Cooler / Valve -> Flash Drum`，补齐单元参数，运行并查看中间流股结果。
6. 放置 `Feed + Feed -> Mixer -> Flash Drum`，补齐双入口、Mixer 参数，运行并查看 Mixer outlet 和 Flash split。
7. 保存、重开、rerun，并确认旧结果失效提示和最新结果入口。
8. 在 readiness 阻断和 Run Panel 正式诊断之间区分建模输入缺口、结构连接问题和求解阶段失败。
...
| package / components | `workspace_document.property_package_choices` / `project_component_choices` |
| 物性到流程图建模 readiness | `StudioGuiWindowPropertyPageModel::flowsheet_modeling_enabled` |
