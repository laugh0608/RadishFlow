...
2026-09-14 项目所有者确认将插件系统、参考 RadishNexus 的分层许可和公共 API 授权 / 配额纳入长期规划。本文承载插件和统一工程接口边界；许可分层见 [认证授权架构](auth-entitlement-architecture.md#软件许可产品授权与操作权限)，凭据与服务访问见 [公共 API 专题](../topics/platform/public-api-and-access-control.md)。这是规划范围确认，不表示插件运行时、许可条款或公网服务已经交付。

2026-09-16 确认设备设计 / 校核为独立工程主线，单位系统与确定性规格 / 建模辅助为近期基础能力；未来 Agent 复用领域入口，当前不接入大模型。Studio 旧主稿的功能分区需重新评审，设计规划见 [UI 专题](studio-ui-topic-plan.md)。本次确认规划与顺序，不冻结接口、存储或依赖选型。

2026-09-16 后续确认补入 [工程设计基础、工况与来源](../topics/engineering-basis-and-cases.md)、目标反算、分层工程检查、压力 / 公用工程、候选权衡与轻量工程交付。先细化共同条件、工况矩阵和来源及其 UI 关系，工程数值和专业检查随模型能力推进；B3-5 无界面建模已接通，U1 单位基础已接通，后续按 U2 / U3 及规格 / 设备依赖推进。

2026-09-17 确认 [颜色与状态语义](studio-visual-system.md#工程字段与状态语义v0-规划) 随 U2 / UI 重设计推进；[流程可视化](../topics/results-review-diagnostics.md#流程可视化与回放规划尚未实现) 按二维流向、数据着色、设备三维及动态回放分期。二维建模为主入口，三维复用同一对象与几何，整厂布局另评估；动画不推进仿真时间，回放依赖真实时间结果，均不等于三维 CFD。当前只确认规划，不新增实现、依赖或项目格式。
...
| 量、单位与单位集 | 规范 SI 值、唯一量 / 单位元数据、显示单位集、输入和输出统一转换 | 普通仿射转换与需上下文的基准转换分开；显示切换不改物理输入 | [单位专题](../topics/units-and-quantity-system.md) 从已有温压流量接入 GUI / CLI / 导出，报告复用 |
| 规格分析与建模辅助 | 就绪、规格冲突、结构自由度、确定性推荐和固定工作流 | 规则由领域拥有；未分析不伪造零自由度；执行复用正式命令 | [辅助专题](../topics/modeling-assistance-and-specifications.md) 先检查 / 推荐，再结构分析；Agent 后续作为消费者 |
| 工程设计基础与工况 | 共同设计条件、输入来源 / 假设、显式覆盖、工况矩阵、工程位号与运行依据 | 工程条件单一来源；同设备跨工况校核保留失败行，历史记录不随当前输入漂移 | [工程基础专题](../topics/engineering-basis-and-cases.md) 从轻量表和来源起步，扩工况 / 复现，不预建审批平台 |
...
| 设备工程任务 | 独立设备计算、初步尺寸、设计、已有设备校核与多工况比较 | 工况、准则、几何、候选和采纳分开；流程耦合需重算和过期检查 | [设备专题](../topics/equipment-design-and-rating.md) 从分离器和换热器起步，不等待动态全部完成 |
...
[单位系统](../topics/units-and-quantity-system.md) 拥有量 / 单位定义与转换真相；变量元数据引用稳定身份，单位集只选择显示口径。界面、结果、报告和外部消费者共用转换入口；普通单位经规范 SI 值转换，表压、质量 / 摩尔和体积基准等需要上下文，焓参考态交由物性能力。纯显示偏好与物理输入修订分开，现有 SI 字段及 CLI 契约不被静默改变。

[规格与辅助](../topics/modeling-assistance-and-specifications.md) 将输入就绪、规格完整性、结构自由度与数值诊断分层。先用确定性规则提供缺项 / 冲突 / 推荐操作及依据；执行前检查身份、修订与前置条件，复用事务。未来 Agent 使用同一能力发现、诊断、动作和结果，不新增绕过约束的写入通道；当前不预建空 AI 模块或聊天入口。

[设备工程](../topics/equipment-design-and-rating.md) 区分工艺计算、初步选型、详细设计和性能校核。独立设备和流程内设备共用模型；推荐候选与已采纳参数分开，方法、工况来源和适用范围可追溯。源工况变化使计算过期；采纳经正式命令，不自动覆盖设备输入。几何可供动态使用，但容积不等于操作积存，动态初值仍需补齐。机械强度、规范和制造结论另有专业范围。

上述能力的用户任务不能被旧画布布局限制。[UI 重设计](studio-ui-topic-plan.md) 先评审功能分区和代表工作流，再按能力切片更新 Pencil 主稿与代码；保持单一工程状态和命令来源。旧稿只在未受影响的既有路径中作参考，新任务没有继承旧定稿状态。

## 工程任务与结果证据

[工程设计基础](../topics/engineering-basis-and-cases.md) 拥有共享条件、假设、来源、工况及覆盖关系，流股 / 设备和任务引用稳定身份；操作条件、设备设计条件和单位显示各有语义。工况记录物理一致的条件组合，有限工况通过不能推导连续操作范围全部可行。

[设计规格求解](../topics/flowsheet-modeling-and-solve.md#设计规格求解规划尚未实现) 根据目标、可调变量、范围和模型反算操作条件；它区别于规格检查和优化。试算使用独立有效输入，采纳才进入文档事务；可行解、最优解、数值失败和已确认冲突分别说明。

[设备专题](../topics/equipment-design-and-rating.md) 负责同几何多工况、控制约束、方案权衡、压力 / 公用工程及所需专业模型。结果可以显示现有可靠条件和缺项，但没有对应模型时不声称已完成网络平衡、供给能力或低负荷运行校核。

[工程检查与交付](../topics/results-review-diagnostics.md#工程检查与交付规划尚未实现) 分开呈现输入 / 规格、数值、守恒、模型适用性和设备约束。轻量流股 / 设备表、假设清单、差异与方法 / 数据 / 运行版本先行；完整报告和不确定度传播分期。物理验证采用分级独立依据，跨软件对照和同源 golden 不单独证明工程准确性。
...
插件是可独立安装、启用、禁用和升级的扩展单元。目标覆盖物性包、单元模型、计算方法、导入导出、结果工具和外部系统连接器；工程对象、命令事务、权限判断和运行生命周期仍由宿主及所属领域拥有，不把所有内建功能强制插件化。

参考 RadishNexus 本地 `docs/architecture/plugin-system.md`（2026-09-14 读取）的渐进扩展、Manifest 权限和 Host API 思路，不迁移其代码、数据库、Web UI 或运行时选型。RadishFlow 需要分别处理数值热路径和一般集成插件：

| 契约 | 规划要求 |
| --- | --- |
| 包与兼容性 | Manifest 声明稳定插件 ID、插件 / Host API 版本、宿主兼容范围、扩展点、模型能力、配置 / 数据版本、许可证和来源校验信息；不支持的组合在装载前拒绝 |
| 安装与权限 | 默认无权限；安装展示项目 / 资产、文件、网络和执行能力范围；管理员或本地所有者明确授予，升级新增权限重新批准 |
| Host API | 通过统一变量 / 动作服务和领域计算契约接入；文档写入复用校验、revision、事务、Undo / Redo 和结果旧化，不直接修改内部对象或存储 |
| 数据与凭据 | 插件配置和数据按插件 / 项目作用域隔离；凭据仅以安全引用或受控使用能力提供，不进入项目、录制脚本或普通 API 响应 |
| 生命周期 | 实例和可变上下文按插件实例 / 任务拥有；运行中禁用或升级需定义停止接收任务、取消 / 排空与资源释放，不能卸载仍被调用的代码 |
| 项目可恢复性 | 记录所需插件 / 模型身份及版本；缺失、禁用、不兼容时保留未知数据和原项目，明确受影响能力，不能静默换模型或丢弃内容后覆盖保存 |
| 故障与资源 | 超时、取消、并发、内存和输出边界可执行；插件失败产生可定位诊断，不提交半成品结果，也不破坏已提交工程数据 |

Manifest 是权限声明，实际安全性需要受控 Host API 与运行时隔离共同保证。任意本机原生代码在宿主进程内具有同等进程权限，不能声称仅靠注册表或签名形成沙箱；签名用于确认来源，不证明代码安全。第三方插件优先评估受限独立进程或 WASM，可信原生数值扩展的进程内装载须单独定义信任边界。独立进程也需限制其操作系统、文件和网络权限，不因进程分离就自动安全。

高频物性与模型求值按模型实例或计算批次设计调用粒度，验证跨边界开销、并发、可重入性和数值一致性，避免每次标量求值都走远端 RPC。C ABI / 消息协议、WASM 与进程模型均为候选，不在规划阶段引入全部运行时。CAPE-OPEN 插件互操作继续经 `.NET` 适配层，不将 COM 类型带入 Rust Core。

推进顺序为真实扩展点、首个可安装插件、公开 SDK、生态分发，阶段退出条件见 [路线图](../radishflow-mvp-roadmap.md#插件与公共-api-的实施切片)。首个插件应由真实物性、单元或导出需求选定。至少验证安装 / 禁用 / 升级、权限拒绝、任务中断、缺插件重开与数据保留；公开 SDK 前还需兼容测试和端到端示例。尚无稳定 Host API 和真实插件消费证据时，不建设完整市场、多语言 SDK 或通用插件管理框架。
...
时间积分和空间离散按模型选择，不在规划阶段锁定单一组合。IDA 的 BDF 与 Radau IIA 隐式 Runge–Kutta 是不同方法；MOL 表示先做空间半离散，不等于正交配置法。有限差分、有限体积与配置法按守恒、平滑性和波传播需求验证；快平衡假设也须保持与动态状态的耦合一致。Rust 语言与稀疏存储选择不代替这些数值验证。参考 [IDA](https://sundials.readthedocs.io/en/latest/ida/Mathematics_link.html)、[Radau](https://docs.scipy.org/doc/scipy/reference/generated/scipy.integrate.Radau.html) 和 [Clawpack](https://www.clawpack.org/)。
...
公共 API 表示供外部消费者使用的稳定接口，不等于匿名接口或公网 HTTP。库、CLI、COM、插件 Host API 和未来远端绑定共享下述领域语义，按各自信任边界校验权限。节点枚举、搜索、订阅、批量读写、任务和结果都应受同一资源权限约束，不能只保护动作入口。凭据、资源授权与计算配额见 [公共 API 与访问控制专题](../topics/platform/public-api-and-access-control.md)。
