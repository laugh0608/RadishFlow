...
| `RadishFlow.CapeOpen.UnitOp.Mvp` | 第一版自有 PMC | 已建立第一版 `net10.0` 最小 PMC 骨架项目，当前提供单个 `ICapeIdentification` + `ICapeUtilities` + `ICapeUnit` 实现类、最小状态机、内部配置入口，以及端口/参数最小占位对象集合与基于对象状态的 `Validate()` 检查；但尚未进入完整 CAPE-OPEN `Collection/Parameter/UnitPort` 实现、native 求解接线、注册或完整 PME 生命周期 |
