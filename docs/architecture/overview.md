...
截至 2026-05-23，Studio Canvas 已补出最小可见、只读扫读层和多单元放置反馈闭环：单元块、物流线、Inspector 焦点高亮、对象列表导航、焦点气泡、material port marker、端口 hover、运行/诊断 badge、状态 legend、viewport focus anchor、对象列表 `All / Attention / Units / Streams` 临时筛选，以及 `Feed / Mixer / Heater / Cooler / Valve / Flash Drum` 的 pending edit palette / commit 路径都通过 GUI-facing presentation 暴露并由 `egui` 渲染。`CommitPendingEditAt -> DocumentCommand::CreateUnit` 成功后会复用新单元的 object command target 和 focus anchor，统一生成 `StudioGuiCanvasCommandResultViewModel`，从而让新建提示、Inspector 焦点、Canvas 一次性定位、GUI activity 与命令面只读反馈走同一条结果路径；无 pending edit、unsupported unit kind、dispatch 失败或 anchor 过期也使用同一套 warning / error result。当前已补多单元 placement 提交端回归矩阵，逐类锁定 `CreateUnit kind`、canonical ports、Inspector 焦点、Canvas focus anchor 和 command result 反馈。2026-05-04 继续在同一边界上补出本地 Canvas suggestions，已能通过正式 `DocumentCommand::ConnectPorts` / outlet stream 创建走通 `Feed -> Flash Drum`、`Feed -> Heater/Cooler/Valve -> Flash Drum` 和 `Feed + Feed -> Mixer -> Flash Drum` 三类可求解建模路径；`Mixer` 多来源场景只在来源数量与未绑定 inlet 数量匹配时给入口建议。同日又补齐真正空白项目的 MVP 前置基线：无组件项目打开时会初始化 MVP 默认二元组件；2026-05-26 后该基线已收口为受控 methane / ethane 目录和内置 `binary-hydrocarbon-lite-v1` package，项目级选择写入 `Flowsheet.components` 与 `Flowsheet.thermo.property_package_id`；2026-05-28 后普通空白项目运行前还必须通过通用 `Flowsheet` readiness，明确要求 Feed source stream 的 T/P/F/z、composition 引用 / 归一和 Heater / Cooler / Valve / Mixer / Flash Drum 必要单元参数都进入文档。随后又补出逐条 suggestion Apply，GUI 可显式接受指定本地建议，而不再只能接受当前 focused suggestion；2026-05-20 suggestion action label 进一步按 acceptance payload 区分为 `Connect stream` / `Create stream`，中文界面显示为 `连接流股` / `创建流股`。2026-05-21 起，`Heater / Cooler / Valve`、`Mixer` 和 `Flash Drum` 的 outlet stream 创建建议必须等必要 inlet 绑定后才出现，避免无自由连线阶段先制造难以修正的 source-only stream。2026-05-22 至 2026-05-23 又补出选中 material stream 后的受控恢复动作：`Disconnect stream` 解除所有材料端口绑定并保留流股规格，`Disconnect source` / `Disconnect sink` 只解除唯一端点，`Delete stream` 解除绑定后删除该流股，`Reconnect stream` 只补齐 source-only / sink-only 流股的唯一缺失端点且过滤会形成 unit dependency cycle 的候选；这些文档变更都进入 `CommandHistory`。空白 `Feed + Feed -> Mixer -> Flash Drum` 路径当前也已有创建、连接、运行、保存、重开、再次运行的 focused 覆盖。Canvas placement 坐标、单元直接拖动位置和 viewport offset 已最小持久化到 `<project>.rfstudio-layout.json` sidecar；空白 Canvas 拖拽只保存 offset，`Fit to content` 只重算 offset，二者都不写 `FlowsheetDocument`、不进文档历史、不保存完整 camera state。当前画布仍只把已有 `FlowsheetDocument` 对象投影到轻量布局状态，定位滚动和高亮属于 shell-local 或 sidecar 状态；端口点击编辑、自由连线创建、任意端口选择、自动布线、完整拖拽布局编辑器、项目 schema 扩张、完整组分数据库、第三方物性包加载、完整物性包浏览器和 CAPE-OPEN 扩张都不属于当前阶段。这一层当前应视为最小建模路径收口，后续不再继续扩 hover、legend、focus 或 command feedback 细节，除非它们直接服务下一条可求解建模闭环。
...
- Studio window-model 当前还会从同一份 `SolveSnapshot` 派生 case-level `review_summary`，按 source / intermediate / terminal streams、latest unit results 和 diagnostics count 汇总结果审阅对象；这只是结果消费 presentation，不成为第二套结果缓存、求解模型或完整报表系统
...
这意味着当前仓库已从“只有 UI 层快照映射桥”推进到“真实 `egui` 壳层可打开已有项目、持久化 shell 级最近项目、运行求解、查看结构化结果/诊断、复制或导出当前快照文本、通过 `Review` 摘要快速核对 source / intermediate / terminal streams 与 latest unit results、定位 Inspector、审阅流股结果与单元最新执行结果、对比当前快照内流股 summary / composition / phase rows、从求解步骤跳转到单元、输入流股和产出流股、从失败摘要触发修复或定位目标、在 Canvas 与 Active Inspector 中查看只读 port attention 摘要、编辑 Stream 字段草稿并提交、选择内置 property package、在左侧项目面板和右侧物性包页选择受控项目组分、编辑并归一化 Feed composition、编辑首批 Unit 参数、执行基础撤销/重做、保存/另存为，并在画布中查看已有单元/流股/端口/诊断状态、执行多单元 pending placement、受控 stream 恢复和 sidecar 级单元位置 / viewport 调整”。最近项目写入独立 Studio preferences 文件，画布筛选、对比选择、focus navigation、layout nudge、viewport offset 与最近一次 command result 展示等状态仍属于 shell 临时状态或 Studio layout sidecar / presentation 反馈，不进入 `*.rfproj.json`。不过这仍是当前可操作工作台基线，不等同于完整跨平台文件工作流、完整应用偏好系统、完整画布编辑器、结果报表系统、跨会话历史持久化或最终视觉方案。
