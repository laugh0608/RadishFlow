...
用途：为 RadishFlow Studio 的首屏、画布、面板、按钮、文字和结果审阅建立一致的 UI 设计规范，作为后续 MVP α UI 重排与实现拆分依据。
...
- 建议以 `Inspector / Results / Run / Entitlement` tab 或等价分段组织。
...
1. 顶部 command bar 定型：保留项目身份、主路径命令和运行状态，清理调试式状态常驻。
2. Canvas header / toolbar 重排：把 Place / suggestion / selection / zoom / legend 分开，长说明进入 tooltip 或底部 Messages。
3. 左侧区域改为 Project / Palette 双入口：项目树和对象库分职责，不再展示完整命令大全。
4. 右侧 Runtime 面板拆成任务化 Inspector：Run、Results、Inspector、Entitlement 只显示当前相关内容。
5. 底部建立 Messages / Run Log / Results drawer：把运行日志、GUI activity 和结果摘要从右侧长列表中迁出。
6. 画布对象视觉升级：先统一 MVP 单元图形、流股标签、选中态和错误 badge，再考虑更多设备符号。
7. 结果审阅表格化：在不改变 `SolveSnapshot` 语义的前提下提供 stream / unit 表格和诊断摘要。

## 验收检查

每轮 UI 改动完成后，至少按下面问题做自检：

- 启动后 5 秒内能否看懂如何打开示例、运行、查看结果、保存？
- 画布是否仍是首屏最大区域？
