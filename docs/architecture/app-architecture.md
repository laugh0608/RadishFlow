...
1. 在已有字段级草稿、单字段提交、多字段批量提交、基础 undo/redo 和保存 / 另存为闭环上，继续明确项目持久化失败提示、覆盖确认和跨平台文件选择策略
...
2. `file.save`、`edit.undo / edit.redo` 后续是否绑定 `Ctrl+S / Ctrl+Z / Ctrl+Y`；若绑定，必须先明确文本输入焦点下的快捷键归属，避免抢走输入控件自己的撤销语义
3. 保存失败恢复、覆盖确认与项目文件原子写入是否需要进入 MVP 第一版，还是先保留为后续持久化治理项
