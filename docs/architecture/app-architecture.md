...
2. 把授权缓存桥接真正接到 `rf-store` 文件读写与应用私有缓存根目录生命周期
3. 把 `PropertyPackageProvider` 与本地缓存目录、`StoredAuthCacheIndex` 和 `rf-thermo` 热路径对接起来
4. 冻结求解入口只由应用层触发，画布层仍只处理几何与交互
5. 决定后续控制面 DTO 是否直接承担 JSON 反序列化，还是由独立 adapter 做协议映射
