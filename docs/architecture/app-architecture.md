...
截至 2026-04-30，Studio 已补出 `DocumentLifecycle(Save / SaveAs)`，保存动作写回 `StoredProjectFile` 并更新 `last_saved_revision`，不进入 `CommandHistory`。

1. 在已有字段级草稿、单字段提交、基础 undo/redo 和保存 / 另存为闭环上，继续明确项目持久化失败提示、覆盖确认和跨平台文件选择策略
2. 明确 Stream Inspector 是否需要多字段批量提交；若需要，应继续复用 `InspectorDraftState` 和正式 document command 边界，而不是在 shell 面板里新增私有缓存
...
2. `InspectorDraftState` 在 MVP 第一版是否需要支持同一面板的多字段批量提交
3. `file.save`、`edit.undo / edit.redo` 后续是否绑定 `Ctrl+S / Ctrl+Z / Ctrl+Y`；若绑定，必须先明确文本输入焦点下的快捷键归属，避免抢走输入控件自己的撤销语义
4. 保存失败恢复、覆盖确认与项目文件原子写入是否需要进入 MVP 第一版，还是先保留为后续持久化治理项
