...
- Studio 当前又已把 canvas suggestion、离散 layout nudge、单元直接拖动与选中流股恢复交互纳入同一条 command surface：`canvas.accept_focused`、`canvas.reject_focused`、`canvas.focus_next`、`canvas.focus_previous`、`canvas.move_selected_unit.*`、直接拖动释放后的 `CanvasUnitLayoutMoveRequested`、`canvas.disconnect_selected_stream` 与 `canvas.delete_selected_stream` 都通过统一 GUI / driver 派发链路处理；layout nudge 和单元拖动只写 `<project>.rfstudio-layout.json` sidecar，不进入项目文档 revision/history
