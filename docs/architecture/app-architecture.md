...
- 当前虽然已有 `WorkspaceRunCommand` 与 `WorkspaceSolveService`，但还没有收口出更完整的运行服务对象模型，例如统一承载结果派发和后续异步执行边界
...
3. 在现有 `WorkspaceRunCommand + WorkspaceSolveService` 基础上，继续收口结果派发与后续异步执行边界
