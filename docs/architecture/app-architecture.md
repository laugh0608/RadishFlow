...
- 由 `WorkspaceSolveService` 负责生成默认 `snapshot_id` / `sequence`，并触发一次显式工作区运行请求
...
- 还没有把这条桥接正式挂到最终桌面命令、按钮或运行服务入口
- 还没有在 `rf-ui` 中冻结“手动运行 / 自动运行 / Hold 恢复”的完整应用事件流
- 当前虽然已有 `WorkspaceSolveService`，但还没有收口出更完整的运行服务对象模型，例如统一承载自动运行策略、包选择和结果派发
...
3. 在现有 `WorkspaceSolveService` 基础上，继续收口自动运行策略、包选择与结果派发边界
