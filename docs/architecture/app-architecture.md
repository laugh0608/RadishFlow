...
- `AppState` 只拥有一个 `WorkspaceState`，不提前做多工作区或多标签容器
...
在真正开始写 `rf-ui` 之前，当前建议把最小状态对象先冻结到以下轮廓：
...
1. 把本页字段级状态定义落到 `rf-ui` 占位代码和类型骨架
2. 把 `DocumentCommand` / `CommandHistoryEntry` 的最小枚举与测试入口补齐
3. 明确 `snapshot_history` 在 `rf-store` 中是只保留内存态还是允许可选持久化
4. 冻结 `InspectorDraftState` 的键模型，避免属性面板后续各自发明草稿缓存
...
2. `snapshot_history` 在 MVP 阶段是否只保留内存态，还是允许项目文件可选持久化
3. `AppLogFeed` 是否只服务 UI 展示，还是也作为后续自动化脚本的导出源
4. `InspectorDraftState` 在 MVP 第一版是否需要支持同一面板的多字段批量提交
