...
- Unit Inspector 只暴露关键参数：`Heater / Cooler` 写回 `outlet_temperature_k`，`Valve` 与 `Flash Drum` 写回 `outlet_pressure_pa`，并通过 `SetUnitParameter` 同步出口模板；Flash Drum 同步两出口模板并作为 TP Flash pressure 使用
...
- `canvas.disconnect_selected_stream` / `canvas.reconnect_selected_stream` / `canvas.delete_selected_stream` 是无自由连线阶段的受控恢复动作：断开保留流股，重连只补齐 source-only 的唯一 inlet 或 sink-only 的唯一 outlet，删除先解除绑定再删除流股；三者进入 `CommandHistory`，但不得扩成端口选择器、自由连线、自动布线或完整拖拽布局
...
- Studio 当前 Run Panel recovery action 必须区分聚焦与修复：前者只定位 target，后者才通过 `run_panel.recover_failure` 执行断开坏引用、删除 orphan stream、创建/绑定 outlet stream 或恢复 canonical port 等 mutation。用户主动选中流股后的断开 / 重连 / 删除走 `canvas.disconnect_selected_stream` / `canvas.reconnect_selected_stream` / `canvas.delete_selected_stream`，不复用 failure-only recovery command
