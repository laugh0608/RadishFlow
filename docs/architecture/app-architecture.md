...
1. 在已有字段级草稿、单字段提交和基础 undo/redo 上，继续冻结保存 / 另存为流程与 `last_saved_revision` 的真实写回口径
...
3. `edit.undo / edit.redo` 后续是否绑定 `Ctrl+Z / Ctrl+Y`；若绑定，必须先明确文本输入焦点下的快捷键归属，避免抢走输入控件自己的撤销语义
