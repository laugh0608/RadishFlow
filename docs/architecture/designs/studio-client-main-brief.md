...
| 中央 Canvas | 中央 Canvas 已移除重复的对象树副本，改为从既有 canvas presentation 派生 `画布状态` 数量概览；画布主体继续渲染工具条、当前选择、视口、图例、单元 / 物料线实体和受控建议，流股 / 单元对象导航由左侧 `项目` 面板和画布实体点击承担 | 自由连线、自动布线、完整项目浏览器、完整拖拽布局、shell 私有对象状态和第二套对象树尚未进入范围 |
...
| 中央画布 | 流程图、单元、流股、端口、标签、画布状态概览、受控 suggestion、当前选择 | 对象树副本、自由连线编辑器、完整自动布线系统 |
