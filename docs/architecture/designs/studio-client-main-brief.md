...
U1 已明确偏好归属：项目单位集作为项目呈现配置随工程保存，个人默认只用于新工程；字段临时输入单位属于草稿，不覆盖项目显示选择。R0 / R1 需分别展示“本次输入单位”和“当前显示单位”的作用范围，并覆盖未编辑文本保持原 SI、非法文本保留原解释、仅显示切换仍有当前结果但呈现设置待保存等状态。完整规则见 [单位偏好设计](../../topics/units-and-quantity-system.md#显示偏好所有权与兼容设计u2-待实现)。本段是交互设计输入，未新增或验收 Pencil 画板。
...
2026-09-17 新增设计输入：U2 首批温压流量字段同时展示 [V0 颜色 / 状态组合](../studio-visual-system.md#工程字段与状态语义v0-规划)。至少覆盖淡蓝可编辑底、深蓝已指定值、深灰计算值、来源标签、草稿、必填缺项、可选空值、指定值带错误、已求解带警告、过期与只读原因；色觉差异、灰度和键盘焦点下仍可识别，单位切换不改变来源或时效。

后续画板按 [V1—V4](../../topics/results-review-diagnostics.md#流程可视化与回放规划尚未实现) 展示流股路径 / 动画开关、带图例数据层、同源设备三维和真实时间回放。V3 须区分非比例示意、候选与已采纳几何；V4 须区分播放时间与求解运行。二维主建模入口保持，三维和整厂布局不挤入本轮 U2 必做画板；本次未修改 `.pen`。
