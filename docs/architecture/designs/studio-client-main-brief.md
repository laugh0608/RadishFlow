...
| 左侧模块 / 项目 | Studio shell 左侧顶层入口已从 `项目 / 示例项目 / 放置` 收敛为 `模块 / 项目`；`模块` 继续消费既有 Canvas place-unit palette、authoring checklist 和 suggestion action，`项目` 继续消费项目对象树、项目物性包 / 组分扫读，并把示例项目保留为可展开入口 | 完整模块库视觉重排、完整分类折叠、搜索实现、自由连线、自动布线和完整拖拽布局尚未进入范围 |
