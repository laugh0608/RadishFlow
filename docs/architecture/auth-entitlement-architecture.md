...
2026-09-14 确认以 RadishNexus 本地 `docs/licensing-strategy.md` 为分层许可的规划参考（读取日期同日）。核心 source-available，Plugin SDK、公共协议、兼容测试和示例拟独立开放源码，官方基础插件拟采用开放源码许可证，社区及付费插件各自声明许可；具体组件许可证、分发边界和授权模板尚待决策。本轮不修改根 [LICENSE](../../LICENSE)，不因规划文字直接授予新的运行、修改或分发权利。

| 层次 | 职责与边界 |
| --- | --- |
| 软件使用许可 | 明确运行、修改、分发和提供服务的权利；以根许可证、组件许可证及有效书面授权为准，API key 不代替使用许可 |
| 产品与资产授权 | 控制受控模型、物性数据、插件或服务的可用范围；已有受控资产租约独立管理，不自动把基础本地运行变为持续联网激活 |
| 操作权限 | 按用户、服务账号或插件身份判断项目、对象、动作与任务权限；认证成功及拥有某项产品授权均不直接授予全部访问权 |
| 资源配额 | 管理请求、任务并发、运行时间、内存和结果大小；运维保护与商业权益分别建模，不用配额字段隐式表达席位许可 |

建议合法授权后的基础本地运行支持离线；若采用签名授权文件，由客户端以公钥验证，签发私钥留在签发设施。受控资产继续遵循自身有效租约。离线撤销存在时差，最长离线期限、重连撤销与到期后的任务行为须在实现前确认；不承诺离线即时撤销。授权异常不删除、加密或扣留用户工程，保存、导出、备份和恢复的保护路径需独立验收。

尚待明确的产品政策：是否完整采用 Nexus 的免费书面授权与无席位限制、授权主体和使用范围、评估路径、期限与终止条件、付费插件安排，以及 SDK / 官方插件的具体许可证。首次对外授权或发布开放组件前完成条款与组件许可边界复核；规划确认不等于这些条款已经生效。

公共 API 的凭据、权限、撤销和限额由 [公共 API 专题](../topics/platform/public-api-and-access-control.md) 承接；账户映射继续由既有账户专题决策。以上分层不替换下文桌面 Bearer、OIDC / PKCE 和物性资产租约契约。
...
- `apps/radishflow-studio` 当前也已补出 `entitlement_session_host`，把 session event、entitlement panel 主动作和指定动作进一步收口为统一宿主触发入口，并显式补出 `NetworkRestored` / `WindowForegrounded` 这类 GUI 生命周期事件到现有 session tick 语义的映射；同时 host 当前已可把 `next_check_at` 收口为可直接 arm 的 timer 请求对象，并进一步产出 `Schedule / Reschedule / Keep / Clear` 这类宿主侧定时器决策；当 runtime 自身没有 entitlement notice 时，宿主可把这类提示覆盖到 panel 视图层，但不回写 `EntitlementState`；host notice 面向用户，只表达“已安排自动授权检查 / 重试”这类可判断摘要，不把 `TimerElapsed`、backoff 细节或 `SystemTime` 结构写入主界面文本；当前又进一步补出 `EntitlementSessionHostSnapshot`，把 schedule、timer arm、timer command、host notice 与 panel driver state 聚合成单一宿主消费对象，并补出 `EntitlementSessionHostContext` 负责保留当前已挂 timer 与上一份 snapshot，让 bootstrap 或后续 GUI 宿主不必再各自重复实现这套上下文推进逻辑
